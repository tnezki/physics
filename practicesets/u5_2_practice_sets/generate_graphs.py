...
# Unit 5 Section 5.2 local graphs
fig, ax = plt.subplots(figsize=(3.5,3.5))
make_context_graph(ax, [
 {'expr': lambda x: np.sin(np.pi*x), 'deriv': lambda x: np.pi*np.cos(np.pi*x), 'color':'steelblue', 'label':'Wave A'},
 {'expr': lambda x: 0.8*np.sin(np.pi*x), 'deriv': lambda x: 0.8*np.pi*np.cos(np.pi*x), 'color':'firebrick', 'label':'Wave B'}],
 0,4,-1.5,1.5,xlabel='Position',ylabel='Displacement',title='Wave Pair')
save_graph(fig,'u5_ps_s2_constructive_sq_v1.png'); plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5,3.5))
make_context_graph(ax, [
 {'expr': lambda x: np.sin(np.pi*x), 'deriv': lambda x: np.pi*np.cos(np.pi*x), 'color':'steelblue', 'label':'Wave A'},
 {'expr': lambda x: -np.sin(np.pi*x), 'deriv': lambda x: -np.pi*np.cos(np.pi*x), 'color':'firebrick', 'label':'Wave B'}],
 0,4,-1.5,1.5,xlabel='Position',ylabel='Displacement',title='Wave Pair')
save_graph(fig,'u5_ps_s2_destructive_sq_v1.png'); plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5,3.5))
make_context_graph(ax, [
 {'expr': lambda x: np.sin(np.pi*x), 'deriv': lambda x: np.pi*np.cos(np.pi*x), 'color':'steelblue', 'label':None}],
 0,6,-1.5,1.5,xlabel='Position',ylabel='Displacement',title='Standing-Wave Snapshot')
save_graph(fig,'u5_ps_s2_standing_snapshot_sq_v1.png'); plt.close(fig)
