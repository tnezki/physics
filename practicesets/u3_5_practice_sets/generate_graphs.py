...
# UNIT 3 SECTION 3.5 PRACTICE-SET GRAPHS
fig, ax = plt.subplots(figsize=(3.5,3.5))
make_context_graph(ax,[{'expr':lambda x:4*x,'deriv':lambda x:np.full_like(x,4.0),'color':'steelblue'}],xmin=0,xmax=5,ymin=0,ymax=20,xlabel='Time (s)',ylabel='Horizontal Position (m)',title='Horizontal Position')
save_graph(fig,'u3_ps_s5_horizontal_position_sq_v1.png')
plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5,3.5))
make_context_graph(ax,[{'expr':lambda x:5*x**2,'deriv':lambda x:10*x,'color':'steelblue'}],xmin=0,xmax=3,ymin=0,ymax=45,xlabel='Time (s)',ylabel='Vertical Drop (m)',title='Vertical Drop')
save_graph(fig,'u3_ps_s5_vertical_drop_sq_v1.png')
plt.close(fig)
