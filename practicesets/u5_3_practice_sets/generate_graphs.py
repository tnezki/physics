...
# Unit 5 Section 5.3 local graphs
fig, ax = plt.subplots(figsize=(3.5,3.5))
make_context_graph(ax, [
 {'expr': lambda x: np.sin(2*np.pi*x), 'deriv': lambda x: 2*np.pi*np.cos(2*np.pi*x), 'color':'steelblue', 'label':'Sound A'},
 {'expr': lambda x: np.sin(4*np.pi*x), 'deriv': lambda x: 4*np.pi*np.cos(4*np.pi*x), 'color':'firebrick', 'label':'Sound B'}],
 0,2,-1.5,1.5,xlabel='Time',ylabel='Relative pressure',title='Sound Waveforms')
save_graph(fig,'u5_ps_s3_pitch_compare_sq_v1.png'); plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5,3.5))
make_context_graph(ax, [
 {'expr': lambda x: 0.6*np.sin(2*np.pi*x), 'deriv': lambda x: 1.2*np.pi*np.cos(2*np.pi*x), 'color':'steelblue', 'label':'Sound A'},
 {'expr': lambda x: 1.2*np.sin(2*np.pi*x), 'deriv': lambda x: 2.4*np.pi*np.cos(2*np.pi*x), 'color':'firebrick', 'label':'Sound B'}],
 0,2,-1.5,1.5,xlabel='Time',ylabel='Relative pressure',title='Sound Waveforms')
save_graph(fig,'u5_ps_s3_loudness_compare_sq_v1.png'); plt.close(fig)
