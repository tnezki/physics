...
# UNIT 4 SECTION 4.4 PRACTICE-SET GRAPHS
fig, ax = plt.subplots(figsize=(3.5,3.5))
make_bar_chart(ax,['PE high','KE high','PE low','KE low'],[80,20,20,80],title='Energy at Two Positions',ylabel='Energy (J)',xlabel='Energy bar')
save_graph(fig,'u4_ps_s4_energy_positions_sq_v1.png')
plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5,3.5))
make_bar_chart(ax,['Initial mech.','Final mech.','Thermal'],[100,70,30],title='Energy Accounting with Friction',ylabel='Energy (J)',xlabel='Energy category')
save_graph(fig,'u4_ps_s4_energy_with_friction_sq_v1.png')
plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5,3.5))
make_context_graph(ax,[
 {'expr':lambda x:100-10*x,'deriv':lambda x:np.full_like(x,-10.0),'color':'steelblue','label':'PE'},
 {'expr':lambda x:10*x,'deriv':lambda x:np.full_like(x,10.0),'color':'firebrick','label':'KE'}],xmin=0,xmax=10,ymin=0,ymax=100,xlabel='Position index',ylabel='Energy (J)',title='Potential and Kinetic Energy Transfer')
save_graph(fig,'u4_ps_s4_ke_pe_transfer_sq_v1.png')
plt.close(fig)
