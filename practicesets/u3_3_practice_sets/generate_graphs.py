...
# UNIT 3 SECTION 3.3 PRACTICE-SET GRAPH
fig, ax = plt.subplots(figsize=(3.5,3.5))
make_context_graph(ax,[{'expr':lambda x:22*(1-np.exp(-0.45*x)),'deriv':lambda x:9.9*np.exp(-0.45*x),'color':'steelblue'}],xmin=0,xmax=10,ymin=0,ymax=24,xlabel='Time (s)',ylabel='Speed (m/s)',title='Falling Speed Over Time')
save_graph(fig,'u3_ps_s3_terminal_speed_sq_v1.png')
plt.close(fig)
