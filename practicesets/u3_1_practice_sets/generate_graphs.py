...
# UNIT 3 SECTION 3.1 PRACTICE-SET GRAPHS
fig, ax = plt.subplots(figsize=(3.5,3.5))
make_context_graph(ax,[{'expr':lambda x:5*x,'deriv':lambda x:np.full_like(x,5.0),'color':'steelblue'}],xmin=0,xmax=8,ymin=0,ymax=40,xlabel='Acceleration (m/s^2)',ylabel='Net Force (N)',title='Force and Acceleration')
save_graph(fig,'u3_ps_s1_force_accel_sq_v1.png')
plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5,3.5))
make_context_graph(ax,[{'expr':lambda x:24/np.maximum(x,0.5),'deriv':lambda x:-24/np.maximum(x,0.5)**2,'color':'steelblue'}],xmin=1,xmax=12,ymin=0,ymax=24,xlabel='Mass (kg)',ylabel='Acceleration (m/s^2)',title='Mass and Acceleration')
save_graph(fig,'u3_ps_s1_mass_accel_sq_v1.png')
plt.close(fig)
