...
# UNIT 8.3 PRACTICE-SET GRAPH GENERATION
if __name__ == "__main__":
    fig, ax = plt.subplots(figsize=(4,3))
    make_context_graph(ax, [
        {"expr": lambda x: 0.2*x, "deriv": lambda x: np.zeros_like(x)+0.2, "color": "steelblue", "label": None}
    ], xmin=0, xmax=10, ymin=0, ymax=2.0, xlabel="Current (A)", ylabel="Magnetic force (N)", title="Fixed wire in a fixed magnetic field")
    save_graph(fig, "force_vs_current.png")
    plt.close(fig)
