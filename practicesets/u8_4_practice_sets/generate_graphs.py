...
# UNIT 8.4 PRACTICE-SET GRAPH GENERATION
if __name__ == "__main__":
    fig, ax = plt.subplots(figsize=(4,3))
    make_context_graph(ax, [
        {"expr": lambda x: 12*np.sin(2*np.pi*x), "deriv": lambda x: 24*np.pi*np.cos(2*np.pi*x), "color": "steelblue", "label": None}
    ], xmin=0, xmax=2, ymin=-15, ymax=15, xlabel="Time (s)", ylabel="Voltage (V)", title="Generator voltage")
    save_graph(fig, "ac_voltage_time.png")
    plt.close(fig)
