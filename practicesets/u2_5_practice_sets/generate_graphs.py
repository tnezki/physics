...
# UNIT 2 SECTION 2.5 PRACTICE-SET GRAPHS
fig, ax = plt.subplots(figsize=(3.5,3.5))
make_context_graph(ax,[{'expr':lambda x:3*x,'deriv':lambda x:np.full_like(x,3.0),'color':'steelblue'}],xmin=0,xmax=8,ymin=0,ymax=24,xlabel='Time (s)',ylabel='Distance (m)',title='Distance Over Time')
save_graph(fig,'u2_ps_s5_dt_constant_sq_v1.png')
plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5,3.5))
make_context_graph(ax,[{'expr':lambda x:0.45*x**2,'deriv':lambda x:0.9*x,'color':'steelblue'}],xmin=0,xmax=8,ymin=0,ymax=30,xlabel='Time (s)',ylabel='Distance (m)',title='Distance Over Time')
save_graph(fig,'u2_ps_s5_dt_speedup_sq_v1.png')
plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5,3.5))
make_context_graph(ax,[{'expr':lambda x:2*x,'deriv':lambda x:np.full_like(x,2.0),'color':'steelblue'}],xmin=0,xmax=6,ymin=0,ymax=12,xlabel='Time (s)',ylabel='Velocity (m/s)',title='Velocity Over Time')
save_graph(fig,'u2_ps_s5_vt_up_sq_v1.png')
plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5,3.5))
make_context_graph(ax,[{'expr':lambda x:12-2*x,'deriv':lambda x:np.full_like(x,-2.0),'color':'steelblue'}],xmin=0,xmax=6,ymin=0,ymax=12,xlabel='Time (s)',ylabel='Velocity (m/s)',title='Velocity Over Time')
save_graph(fig,'u2_ps_s5_vt_down_sq_v1.png')
plt.close(fig)
