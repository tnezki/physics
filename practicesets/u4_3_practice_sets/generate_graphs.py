...
# UNIT 4 SECTION 4.3 PRACTICE-SET GRAPHS
fig, ax = plt.subplots(figsize=(3.5,3.5))
make_context_graph(ax,[{'expr':lambda x:15*x,'deriv':lambda x:np.full_like(x,15.0),'color':'steelblue'}],xmin=0,xmax=8,ymin=0,ymax=120,xlabel='Displacement (m)',ylabel='Work (J)',title='Work for a Constant 15 N Force')
save_graph(fig,'u4_ps_s3_work_distance_sq_v1.png')
plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5,3.5))
make_context_graph(ax,[{'expr':lambda x:x**2,'deriv':lambda x:2*x,'color':'steelblue'}],xmin=0,xmax=10,ymin=0,ymax=100,xlabel='Speed (m/s)',ylabel='Kinetic Energy (J)',title='Kinetic Energy for a Fixed Mass')
save_graph(fig,'u4_ps_s3_ke_speed_sq_v1.png')
plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5,3.5))
make_bar_chart(ax,['A','B','C'],[60,100,75],title='Power Comparison',ylabel='Power (W)',xlabel='Machine')
save_graph(fig,'u4_ps_s3_power_compare_sq_v1.png')
plt.close(fig)
