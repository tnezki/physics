...
# Unit 5 Section 5.4 local graphs
fig, ax = plt.subplots(figsize=(3.5,3.5))
make_context_graph(ax, [
 {'expr': lambda x: 0.2 + 4*np.exp(-((x-5.0)/1.2)**2), 'deriv': lambda x: 4*np.exp(-((x-5.0)/1.2)**2)*(-2*(x-5.0)/(1.2**2)), 'color':'steelblue', 'label':None}],
 0,10,0,5,xlabel='Driving frequency',ylabel='Response amplitude',title='Driven Response')
save_graph(fig,'u5_ps_s4_resonance_response_sq_v1.png'); plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5,3.5))
make_context_graph(ax, [
 {'expr': lambda x: 170*x, 'deriv': lambda x: 170+0*x, 'color':'steelblue', 'label':None}],
 0,1.2,0,220,xlabel='Echo time (s)',ylabel='One-way distance (m)',title='Echo Timing')
save_graph(fig,'u5_ps_s4_echo_distance_sq_v1.png'); plt.close(fig)
