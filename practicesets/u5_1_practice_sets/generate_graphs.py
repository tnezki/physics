...
# Unit 5 Section 5.1 local graphs
fig, ax = plt.subplots(figsize=(3.5,3.5))
make_context_graph(ax, [
 {'expr': lambda x: 0.8*np.sin(np.pi*x/2), 'deriv': lambda x: 0.4*np.pi*np.cos(np.pi*x/2), 'color':'steelblue', 'label':'Wave A'},
 {'expr': lambda x: 1.6*np.sin(np.pi*x/2), 'deriv': lambda x: 0.8*np.pi*np.cos(np.pi*x/2), 'color':'firebrick', 'label':'Wave B'}],
 0,8,-2,2,xlabel='Position (m)',ylabel='Displacement',title='Wave Comparison')
save_graph(fig,'u5_ps_s1_amplitude_compare_sq_v1.png'); plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5,3.5))
make_context_graph(ax, [
 {'expr': lambda x: np.sin(2*np.pi*x), 'deriv': lambda x: 2*np.pi*np.cos(2*np.pi*x), 'color':'steelblue', 'label':'Wave A'},
 {'expr': lambda x: np.sin(4*np.pi*x), 'deriv': lambda x: 4*np.pi*np.cos(4*np.pi*x), 'color':'firebrick', 'label':'Wave B'}],
 0,2,-1.5,1.5,xlabel='Time (s)',ylabel='Displacement',title='Frequency Comparison')
save_graph(fig,'u5_ps_s1_frequency_compare_sq_v1.png'); plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5,3.5))
make_context_graph(ax, [
 {'expr': lambda x: np.sin(np.pi*x), 'deriv': lambda x: np.pi*np.cos(np.pi*x), 'color':'steelblue', 'label':None}],
 0,4,-1.5,1.5,xlabel='Time (s)',ylabel='Displacement',title='Wave Trace')
save_graph(fig,'u5_ps_s1_period_trace_sq_v1.png'); plt.close(fig)
