...
# Unit 5.1 practice graph: periodic wave profile
fig, ax = plt.subplots(figsize=(4,3))
make_context_graph(ax,[{'expr':lambda x:2*np.sin(np.pi*x/2),'deriv':lambda x:np.pi*np.cos(np.pi*x/2),'color':'steelblue','label':None}],xmin=0,xmax=8,ymin=-3,ymax=3,xlabel='Position (m)',ylabel='Displacement (cm)',title='')
save_graph(fig,'p1_wave_profile.png')
plt.close(fig)
