...
# Unit 5 Section 5.5 local graphs
fig, ax = plt.subplots(figsize=(3.5,3.5))
make_context_graph(ax, [
 {'expr': lambda x: np.sin(2*np.pi*4*x)+np.sin(2*np.pi*4.5*x), 'deriv': lambda x: 8*np.pi*np.cos(8*np.pi*x)+9*np.pi*np.cos(9*np.pi*x), 'color':'steelblue', 'label':None}],
 0,4,-2.5,2.5,xlabel='Time (s)',ylabel='Resultant displacement',title='Combined Sound')
save_graph(fig,'u5_ps_s5_beat_pattern_sq_v1.png'); plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5,3.5))
make_context_graph(ax, [
 {'expr': lambda x: np.sin(2*np.pi*x), 'deriv': lambda x: 2*np.pi*np.cos(2*np.pi*x), 'color':'steelblue', 'label':'Wave A'},
 {'expr': lambda x: -np.sin(2*np.pi*x), 'deriv': lambda x: -2*np.pi*np.cos(2*np.pi*x), 'color':'firebrick', 'label':'Wave B'}],
 0,2,-1.5,1.5,xlabel='Time',ylabel='Displacement',title='Phase Comparison')
save_graph(fig,'u5_ps_s5_interference_phase_sq_v1.png'); plt.close(fig)
