...
# --- Unit 2.3 practice-set graphs ---
def _save_u23_vt(filename, v0, a, xmax, ymax):
    fig, ax = plt.subplots(figsize=(3.5,3.5))
    make_context_graph(ax,[{'expr':lambda x,v0=v0,a=a:v0+a*x,'deriv':lambda x,a=a:np.full_like(x,a,dtype=float),'color':'steelblue','label':None}],0,xmax,0,ymax,xlabel='Time (s)',ylabel='Velocity (m/s)',title='')
    save_graph(fig, filename); plt.close(fig)
_save_u23_vt('p1_velocity_time.png',2,3,5,18)
_save_u23_vt('extra_velocity_time.png',1,2,6,14)
