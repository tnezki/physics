...
# UNIT 2 SECTION 2.3 PRACTICE-SET GRAPHS
fig, ax = plt.subplots(figsize=(3.5,3.5))
make_context_graph(ax,[{'expr':lambda x:2*x+1,'deriv':lambda x:np.full_like(x,2.0),'color':'steelblue'}],xmin=0,xmax=6,ymin=0,ymax=14,xlabel='Time (s)',ylabel='Velocity (m/s)',title='Velocity Over Time')
save_graph(fig,'u2_ps_s3_velocity_up_sq_v1.png')
plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5,3.5))
make_context_graph(ax,[{'expr':lambda x:12-2*x,'deriv':lambda x:np.full_like(x,-2.0),'color':'steelblue'}],xmin=0,xmax=6,ymin=0,ymax=14,xlabel='Time (s)',ylabel='Velocity (m/s)',title='Velocity Over Time')
save_graph(fig,'u2_ps_s3_velocity_down_sq_v1.png')
plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5,3.5))
make_context_graph(ax,[{'expr':lambda x:np.full_like(x,7.0),'deriv':lambda x:np.zeros_like(x),'color':'steelblue'}],xmin=0,xmax=6,ymin=0,ymax=12,xlabel='Time (s)',ylabel='Velocity (m/s)',title='Velocity Over Time')
save_graph(fig,'u2_ps_s3_velocity_constant_sq_v1.png')
plt.close(fig)
