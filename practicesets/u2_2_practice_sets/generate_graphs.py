...
# --- Unit 2.2 practice-set graphs ---
def _save_u22_line(filename, speed, xmax, ymax):
    fig, ax = plt.subplots(figsize=(3.5,3.5))
    make_context_graph(ax,[{'expr':lambda x,s=speed:s*x,'deriv':lambda x,s=speed:np.full_like(x,s,dtype=float),'color':'steelblue','label':None}],0,xmax,0,ymax,xlabel='Time (s)',ylabel='Distance (m)',title='')
    save_graph(fig, filename); plt.close(fig)
_save_u22_line('p1_distance_time.png',4,10,40)
_save_u22_line('extra_distance_time.png',6,8,48)
