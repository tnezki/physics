...
# UNIT 2 SECTION 2.2 PRACTICE-SET GRAPHS
fig, ax = plt.subplots(figsize=(3.5,3.5))
make_context_graph(ax,[{'expr':lambda x:4*x,'deriv':lambda x:np.full_like(x,4.0),'color':'steelblue'}],xmin=0,xmax=8,ymin=0,ymax=32,xlabel='Time (s)',ylabel='Distance (m)',title='Distance Over Time')
save_graph(fig,'u2_ps_s2_distance_time_sq_v1.png')
plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5,3.5))
make_context_graph(ax,[{'expr':lambda x:np.full_like(x,18.0),'deriv':lambda x:np.zeros_like(x),'color':'steelblue'}],xmin=0,xmax=8,ymin=0,ymax=30,xlabel='Time (s)',ylabel='Distance (m)',title='Distance Over Time')
save_graph(fig,'u2_ps_s2_rest_time_sq_v1.png')
plt.close(fig)
