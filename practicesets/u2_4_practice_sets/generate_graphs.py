...
# UNIT 2 SECTION 2.4 PRACTICE-SET GRAPHS
fig, ax = plt.subplots(figsize=(3.5,3.5))
make_context_graph(ax,[{'expr':lambda x:10*x,'deriv':lambda x:np.full_like(x,10.0),'color':'steelblue'}],xmin=0,xmax=5,ymin=0,ymax=50,xlabel='Time (s)',ylabel='Speed (m/s)',title='Free-Fall Speed')
save_graph(fig,'u2_ps_s4_freefall_velocity_sq_v1.png')
plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5,3.5))
make_context_graph(ax,[{'expr':lambda x:5*x**2,'deriv':lambda x:10*x,'color':'steelblue'}],xmin=0,xmax=5,ymin=0,ymax=125,xlabel='Time (s)',ylabel='Distance Fallen (m)',title='Free-Fall Distance')
save_graph(fig,'u2_ps_s4_freefall_distance_sq_v1.png')
plt.close(fig)
