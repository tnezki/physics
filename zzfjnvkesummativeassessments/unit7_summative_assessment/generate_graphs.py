...
# =============================================================================
# UNIT 7 SUMMATIVE GRAPH GENERATION CALLS — APPENDED BELOW AUTHORITATIVE SOURCE
# =============================================================================

def _u7_save_vi(filename, funcs, xmax, ymax):
    fig, ax = plt.subplots(figsize=(3.5,3.5))
    make_context_graph(ax, funcs, xmin=0, xmax=xmax, ymin=0, ymax=ymax, xlabel='Current (A)', ylabel='Voltage (V)', title='')
    save_graph(fig, filename)
    plt.close(fig)

_u7_save_vi('u7_v1_vi.png',[{'expr':lambda x:4*x,'deriv':lambda x:np.full_like(x,4.0),'color':'steelblue','label':None}],4,16)
_u7_save_vi('u7_v2_vi.png',[{'expr':lambda x:8*x,'deriv':lambda x:np.full_like(x,8.0),'color':'steelblue','label':None}],3,24)
_u7_save_vi('u7_v3_vi.png',[{'expr':lambda x:3*x,'deriv':lambda x:np.full_like(x,3.0),'color':'steelblue','label':'Device A'},{'expr':lambda x:6*x,'deriv':lambda x:np.full_like(x,6.0),'color':'firebrick','label':'Device B'}],4,24)
_u7_save_vi('u7_v4_vi.png',[{'expr':lambda x:2*x+0.5*x*x,'deriv':lambda x:2+1.0*x,'color':'steelblue','label':None}],4,16)
_u7_save_vi('u7_v5_vi.png',[{'expr':lambda x:5*x,'deriv':lambda x:np.full_like(x,5.0),'color':'steelblue','label':None}],4,20)
_u7_save_vi('u7_v6_vi.png',[{'expr':lambda x:1.5*x+0.8*x*x,'deriv':lambda x:1.5+1.6*x,'color':'steelblue','label':None}],4,20)
_u7_save_vi('u7_v7_vi.png',[{'expr':lambda x:2*x,'deriv':lambda x:np.full_like(x,2.0),'color':'steelblue','label':'Device A'},{'expr':lambda x:5*x,'deriv':lambda x:np.full_like(x,5.0),'color':'firebrick','label':'Device B'}],5,25)
_u7_save_vi('u7_v8_vi.png',[{'expr':lambda x:4*x,'deriv':lambda x:np.full_like(x,4.0),'color':'steelblue','label':None}],5,20)
