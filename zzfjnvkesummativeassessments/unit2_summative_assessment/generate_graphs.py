...
# =============================================================================
# UNIT 2 SUMMATIVE GRAPH GENERATION — APPENDED BELOW AUTHORITATIVE SOURCE
# =============================================================================

def _u2_ctx(filename, f, fp, xmin, xmax, ymin, ymax, xlabel, ylabel):
    fig, ax = plt.subplots(figsize=(3.5,3.0))
    fig.patch.set_facecolor('white')
    make_context_graph(ax,[{'expr':f,'deriv':fp,'color':'steelblue'}],xmin,xmax,ymin,ymax,xlabel=xlabel,ylabel=ylabel,title='Motion data')
    save_graph(fig,filename)
    plt.close(fig)

def _u2_make(kind, filename, p):
    if kind=='linear_d':
        m=p['m']; _u2_ctx(filename,lambda x,m=m:m*x,lambda x,m=m:np.full_like(x,m,dtype=float),0,p['xmax'],0,p['ymax'],'Time (s)','Distance (m)')
    elif kind=='quad_d':
        a=p['a']; _u2_ctx(filename,lambda x,a=a:a*x**2,lambda x,a=a:2*a*x,0,p['xmax'],0,p['ymax'],'Time (s)','Distance (m)')
    elif kind=='linear_v':
        b=p.get('b',0); m=p['m']; _u2_ctx(filename,lambda x,b=b,m=m:b+m*x,lambda x,m=m:np.full_like(x,m,dtype=float),0,p['xmax'],p.get('ymin',0),p['ymax'],'Time (s)','Velocity (m/s)')
    elif kind=='freefall_speed':
        m=p['m']; _u2_ctx(filename,lambda x,m=m:m*x,lambda x,m=m:np.full_like(x,m,dtype=float),0,p['xmax'],0,p['ymax'],'Time (s)','Speed (m/s)')
    elif kind=='const_v':
        c=p['c']; _u2_ctx(filename,lambda x,c=c:np.full_like(x,c,dtype=float),lambda x:np.zeros_like(x,dtype=float),0,p['xmax'],p.get('ymin',0),p['ymax'],'Time (s)','Velocity (m/s)')
    elif kind=='cross_v':
        b=p['b']; m=p['m']; _u2_ctx(filename,lambda x,b=b,m=m:b+m*x,lambda x,m=m:np.full_like(x,m,dtype=float),0,p['xmax'],p['ymin'],p['ymax'],'Time (s)','Velocity (m/s)')
    elif kind=='move_stop_move':
        def f(x):
            return np.piecewise(x,[x<=4,(x>4)&(x<=6),x>6],[lambda z:3*z,12,lambda z:12+5*(z-6)])
        def fp(x):
            return np.piecewise(x,[x<=4,(x>4)&(x<=6),x>6],[3,0,5]).astype(float)
        _u2_ctx(filename,f,fp,0,p['xmax'],0,p['ymax'],'Time (s)','Distance (m)')
    elif kind=='slow_d':
        _u2_ctx(filename,lambda x:10*x-0.5*x**2,lambda x:10-x,0,p['xmax'],0,p['ymax'],'Time (s)','Distance (m)')

_u2_make('linear_d', 'u2_sum_q9_v1.png', {'m': 3, 'xmax': 6, 'ymax': 20})
_u2_make('quad_d', 'u2_sum_q9_v2.png', {'a': 0.6, 'xmax': 6, 'ymax': 24})
_u2_make('linear_v', 'u2_sum_q9_v3.png', {'b': 1, 'm': 2, 'xmax': 6, 'ymin': 0, 'ymax': 14})
_u2_make('const_v', 'u2_sum_q9_v4.png', {'c': 5, 'xmax': 6, 'ymin': 0, 'ymax': 10})
_u2_make('move_stop_move', 'u2_sum_q9_v5.png', {'xmax': 9, 'ymax': 30})
_u2_make('cross_v', 'u2_sum_q9_v6.png', {'b': 6, 'm': -2, 'xmax': 6, 'ymin': -6, 'ymax': 8})
_u2_make('slow_d', 'u2_sum_q9_v7.png', {'xmax': 8, 'ymax': 45})
_u2_make('linear_v', 'u2_sum_q9_v8.png', {'b': 12, 'm': -3, 'xmax': 4, 'ymin': 0, 'ymax': 14})
_u2_make('linear_v', 'u2_sum_q14_v1.png', {'b': 0, 'm': 2, 'xmax': 4, 'ymin': 0, 'ymax': 10})
_u2_make('linear_d', 'u2_sum_q14_v2.png', {'m': 4, 'xmax': 5, 'ymax': 22})
_u2_make('quad_d', 'u2_sum_q14_v3.png', {'a': 0.8, 'xmax': 5, 'ymax': 22})
_u2_make('const_v', 'u2_sum_q14_v4.png', {'c': 5, 'xmax': 5, 'ymin': 0, 'ymax': 10})
_u2_make('linear_v', 'u2_sum_q14_v5.png', {'b': 10, 'm': -2, 'xmax': 4, 'ymin': 0, 'ymax': 12})
_u2_make('move_stop_move', 'u2_sum_q14_v6.png', {'xmax': 8, 'ymax': 24})
_u2_make('linear_v', 'u2_sum_q14_v7.png', {'b': 1, 'm': 2, 'xmax': 4, 'ymin': 0, 'ymax': 11})
_u2_make('slow_d', 'u2_sum_q14_v8.png', {'xmax': 6, 'ymax': 36})
_u2_make('freefall_speed', 'u2_sum_q16_v3.png', {'m': 10, 'xmax': 4, 'ymax': 45})
_u2_make('cross_v', 'u2_sum_q16_v6.png', {'b': 8, 'm': -2, 'xmax': 7, 'ymin': -8, 'ymax': 10})
_u2_make('cross_v', 'u2_sum_q16_v8.png', {'b': 6, 'm': -1.5, 'xmax': 7, 'ymin': -6, 'ymax': 8})
_u2_make('linear_v', 'u2_sum_fr20_v1.png', {'b': 2, 'm': 2, 'xmax': 5, 'ymin': 0, 'ymax': 14})
_u2_make('move_stop_move', 'u2_sum_fr20_v2.png', {'xmax': 9, 'ymax': 30})
_u2_make('linear_d', 'u2_sum_fr20_v4.png', {'m': 4, 'xmax': 6, 'ymax': 26})
_u2_make('const_v', 'u2_sum_fr20_v5.png', {'c': 12, 'xmax': 8, 'ymin': 0, 'ymax': 16})
_u2_make('cross_v', 'u2_sum_fr20_v6.png', {'b': 6, 'm': -2, 'xmax': 6, 'ymin': -6, 'ymax': 8})
_u2_make('quad_d', 'u2_sum_fr20_v8.png', {'a': 0.7, 'xmax': 6, 'ymax': 28})
