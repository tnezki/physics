...
# =============================================================================
# UNIT 8 SUMMATIVE GRAPH GENERATION CALLS — APPENDED BELOW AUTHORITATIVE SOURCE
# =============================================================================
def _u8_sum_ac(filename, amp, freq, xmax, phase=0.0):
    fig, ax = plt.subplots(figsize=(4.2,3.2))
    yb=12 if amp<=10 else 20
    make_context_graph(ax,[{'expr':lambda x,A=amp,f=freq,p=phase:A*np.sin(2*np.pi*f*x+p),
                            'deriv':lambda x,A=amp,f=freq,p=phase:A*2*np.pi*f*np.cos(2*np.pi*f*x+p),
                            'color':'steelblue','label':None}],
                       xmin=0,xmax=xmax,ymin=-yb,ymax=yb,
                       xlabel='Time (s)',ylabel='Voltage (V)',title='Generator Voltage vs. Time')
    save_graph(fig,filename); plt.close(fig)
_u8_sum_ac('u8_sum_v1_ac.png',10,1.0,3.0,0.0)
_u8_sum_ac('u8_sum_v2_ac.png',8,0.5,4.0,0.0)
_u8_sum_ac('u8_sum_v3_ac.png',12,2.0,2.0,0.0)
_u8_sum_ac('u8_sum_v4_ac.png',10,1.0,2.0,-np.pi/2)
_u8_sum_ac('u8_sum_v5_ac.png',6,1.0,2.0,0.0)
_u8_sum_ac('u8_sum_v6_ac.png',10,0.75,4.0,0.0)
_u8_sum_ac('u8_sum_v7_ac.png',9,2.0,1.5,0.0)
_u8_sum_ac('u8_sum_v8_ac.png',9,0.5,6.0,0.0)
