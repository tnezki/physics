...
# =============================================================================
# PROGRESS TRACKER FIGURES - UNIT 8
# =============================================================================
fig, ax = plt.subplots(figsize=(3.5, 3.0))
make_context_graph(
    ax,
    [{
        'expr': lambda x: 10*np.abs(np.sin(2*np.pi*x)),
        'deriv': lambda x: 20*np.pi*np.cos(2*np.pi*x)*np.sign(np.sin(2*np.pi*x)),
        'color': 'steelblue',
        'label': None,
    }],
    xmin=0, xmax=1, ymin=0, ymax=10,
    xlabel='Time (one rotation)', ylabel='Voltage (V)',
    title='Student AC generator graph'
)
save_graph(fig, 'u8_pt_ac_flawed_01.png')
plt.close(fig)
