...
# Unit 1 Progress Tracker graph-generation blocks
# All figures use graph_tool.py functions. Styling is not overridden.

_save_standard_blank('u1_pt_blank_grid_01.png')

fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_context_graph(ax, [
    {'expr': lambda x: 6 + 3*x, 'deriv': lambda x: 0*x + 3, 'color': 'steelblue', 'label': None}
], xmin=0, xmax=8, ymin=0, ymax=35, xlabel='Input', ylabel='Output', title='')
save_graph(fig, 'u1_pt_read_context_01.png')
plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_scatter_plot(ax, [0, 1, 2, 2, 3], [2, 4, 6, 9, 8], xmin=0, xmax=4, ymin=0, ymax=10, color='steelblue', point_size=40, line_of_best_fit=False, xlabel='Input', ylabel='Output', title='')
save_graph(fig, 'u1_pt_relation_points_01.png')
plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_standard_graph(ax, [
    {'expr': lambda x: 2*x + 5, 'deriv': lambda x: 0*x + 2, 'color': 'steelblue', 'label': None}
], title='')
save_graph(fig, 'u1_pt_mismatch_line_01.png')
plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_context_graph(ax, [
    {'expr': lambda x: 20 + 2*x, 'deriv': lambda x: 0*x + 2, 'color': 'steelblue', 'label': 'Plan A'},
    {'expr': lambda x: 8 + 5*x, 'deriv': lambda x: 0*x + 5, 'color': 'firebrick', 'label': 'Plan B'},
], xmin=0, xmax=14, ymin=0, ymax=80, xlabel='Items', ylabel='Cost', title='')
save_graph(fig, 'u1_pt_plan_compare_01.png')
plt.close(fig)
