#!/usr/bin/env python3
# Universal Bank Finalizer v6.5 SAFE — Physics duplicate/security/figure-answer-neutrality hardened; embeds authoritative MathJax v10 repair engine
...
.utility-links { margin: 4px 0 8px; font-size: 13px; }
.utility-links a { color: #1f3864; }
.bank-section { margin: 0 0 30px; }
.resource-group { margin: 0; padding: 0; }
.resource-group > h3 { margin-top: 24px; margin-bottom: 8px; }
.review-target { margin: 0 0 34px; }
.review-family { margin: 18px 0 28px; padding-top: 8px; border-top: 1px solid #d7dce5; }
.review-family > h3 { margin-bottom: 8px; }
.review-grid {
  display: grid;
...
.review-grid .bank-item { margin: 0; min-width: 0; }
.review-view-note { font-size: 13px; color: #596579; }
.bank-item {
...
.meta { margin-bottom: 8px; font-size: 12px; color: #596579; line-height: 1.55; }
.meta-ican-text { color: #3e4a5c; }
.audit-status { display: inline-block; padding: 1px 6px; border: 1px solid transparent; border-radius: 999px; font-weight: 700; white-space: nowrap; }
.audit-needs-audit { color: #8b1e1e; background: #fff0f0; border-color: #e2b6b6; }
.audit-keep { color: #2f6b36; background: #eff8f0; border-color: #b8d7bb; }
.audit-clean { color: #795b00; background: #fff8dd; border-color: #e6d28a; }
.audit-rebuild { color: #8b1e1e; background: #ffe4e4; border-color: #d99999; }
.resource-sequence-view .bank-item { display: block; width: auto; }
.resource-sequence-view > section { margin-bottom: 34px; }
...
REQUIRED_SUFFIXES=['_bank_data.json','_downstream_manifest.json','_i_can_map.csv','_mapping.csv','_evidence_map.csv','_summative_source_map.csv','_question_design_map.json']
JUNK_NAMES={'.DS_Store'}
TAG_RE=re.compile(r'<[^>]+>')
...
FIGURE_ANSWER_SENSITIVE_RE=re.compile(
    r'\b(?:identify|name|which|what force|missing force|label|infer|rank|order|calculate|compute|predict|critique|diagnose|construct|draw|determine|find the direction|find direction|find the value|what value|what direction|where should|which arrow|which ray|which path)\b', re.I
)

def _numeric_shell(text):
    """Normalize numbers/units enough to surface heavy number-only variants without treating them as exact duplicates."""
    s=normalize_prompt(text)
    s=re.sub(r'(?<![a-z])[-+]?\d+(?:\.\d+)?(?:\s*[x×]\s*10\s*\^?\s*[-+]?\d+)?', '<num>', s)
    s=re.sub(r'\b(?:n|kg|g|m|cm|mm|km|s|ms|h|hr|mph|m/s|m/s2|m/s\^2|j|w|v|a|ohm|hz|db|c)\b','<unit>',s)
    return s

PHYSICS_GENERIC_OBJECT_RE=re.compile(
    r'\b(?:object|cart|wagon|sled|box|crate|book|backpack|suitcase|planter|basket|puck|ball|'
    r'train|boat|car|truck|bicycle|bike|elevator|person|student|worker|equipment case|sign|block|'
    r'package|skater|runner|vehicle|toy|cabinet|table|chair|lamp|rope)\b', re.I
)
PHYSICS_HIGH_CONFIDENCE_CONTRADICTIONS=[
    re.compile(r'\b(?:moving|travels?|rolling|slides?|glides?)\b.{0,80}\b(?:steady|constant)\s+(?:speed|velocity)\b.{0,80}\b(?:is|remains?)\s+at\s+rest\b', re.I),
    re.compile(r'\b(?:is|remains?)\s+at\s+rest\b.{0,80}\b(?:moves?|moving|travels?|rolling|slides?|glides?)\b.{0,80}\b(?:steady|constant)\s+(?:speed|velocity)\b', re.I),
    re.compile(r'\bstanding\s+still\b.{0,80}\b(?:moves?|moving|travels?)\s+straight\b', re.I),
]
PHYSICS_CONTEXT_ODDITY_RE=re.compile(r'\bhanging\b.{0,50}\bresting\s+on\s+(?:a\s+)?(?:floor|table)\b',re.I)
BAD_ARTICLE_RE=re.compile(r'\ba\s+(?:elevator|equipment|air-hockey|air\b|object\b|incline\b|experiment\b|electric\b|ice\b)',re.I)
REPRESENTATION_EXPLICIT_RE=re.compile(
    r'\b(?:diagram|graph|table|vector diagram|vector model|arrow length|arrow direction|force model|'
    r'free[- ]body|ray diagram|field lines?|circuit diagram|sketch|draw|plot|data table)\b', re.I
)

def _physics_semantic_shell(text):
    s=_numeric_shell(text)
    s=PHYSICS_GENERIC_OBJECT_RE.sub('<object>',s)
    # Remove common superficial lead-ins that often carry irrelevant variation-only givens.
    s=re.sub(r'\bthe <object> in this situation has mass <num> <unit>[.!?]?\s*','',s)
    s=re.sub(r'\ba controlled measurement of <object>[^.!?]{0,120}?collected over <num> <unit>[.!?]?\s*','',s)
    return WS_RE.sub(' ',s).strip()

def _actual_representation_types(item):
    kinds=set()
    htmlv=str(item.get('student_html') or '')
    figs=item.get('figures') or []
    tables=item.get('tables') or []
    if figs or re.search(r'<(?:img|svg)\b',htmlv,re.I): kinds.add('figure')
    if tables or re.search(r'<table\b',htmlv,re.I): kinds.add('table')
    # A deliberately drawn inline vector/arrow model can satisfy simple vector representation evidence.
    visible=html.unescape(TAG_RE.sub(' ',htmlv))
    if re.search(r'[←→↑↓↔↕]|\b(?:arrow|vector)\b.{0,30}[←→↑↓]',visible,re.I): kinds.add('vector')
    return kinds

def _has_actual_representation(item):
    return bool(_actual_representation_types(item))

def _source_basis_key(item):
    v=item.get('source_basis')
    if isinstance(v,(list,tuple)):
        return ' | '.join(clean_text(x) for x in v if clean_text(x)).lower()
    return clean_text(v).lower()

def _intentional_exact_repeat_reason(item):
    return clean_text(item.get('intentional_exact_repeat_reason') or item.get('intentional_repeat_reason')).strip()

def _is_secure(item):
    sec=clean_text(item.get('security') or item.get('security_level') or item.get('data_security')).lower()
    dest=clean_text(item.get('destination') or item.get('destination_group')).lower()
    return bool(item.get('summative_version')) or 'secure' in sec or 'summative' in dest or 'exit' in dest
...
def checks(base,residual_math):
    ...
    req=[base/f'unit{unit}.html',base/f'unit{unit}_review.html',base/f'unit{unit}_resources.html',base/'generate_graphs.py',base/'~finalize_bank.py']+[base/f'unit{unit}{s}' for s in REQUIRED_SUFFIXES]
    ...
    try:
        design=json.loads((base/f'unit{unit}_question_design_map.json').read_text(encoding='utf-8'))
    except Exception as e:
        design={};hard.append({'where':'QUESTION DESIGN MAP','issue':f'Cannot parse unit{unit}_question_design_map.json: {e}'})
    design_slots=set()
    if design:
        if str(design.get('schema_version') or '')!='question-design-map-v1':
            hard.append({'where':'QUESTION DESIGN MAP','issue':f'Unexpected schema_version {design.get("schema_version")!r}; expected question-design-map-v1.'})
        for sec in design.get('sections') or []:
            for destplan in sec.get('destination_designs') or []:
                for slot in destplan.get('planned_slots') or []:
                    sid=clean_text(slot.get('slot_id')).strip()
                    if not sid:
                        hard.append({'where':'QUESTION DESIGN MAP','issue':'A planned item slot is missing slot_id.'})
                    elif sid in design_slots:
                        hard.append({'where':'QUESTION DESIGN MAP','issue':f'Duplicate planned slot_id {sid!r}.'})
                    else:
                        design_slots.add(sid)
    counts['question_design_slots']=len(design_slots)
    if family=='science':
        required_literal={'bank_id','destination','section','student_html','student_text','choices','answer_text','tables','figures','learning_target','i_can','i_can_id','audit_status','supporting_i_can_ids','mastery_goal_id','evidence_point_id','security_level','dok','question_family_id','variation','source_basis','question_design'}
        schema_missing=defaultdict(list)
        schema_blank=defaultdict(list)
        for item in items:
            bid=str(item.get('bank_id') or '<missing-id>')
            for fld in required_literal:
                if fld not in item:
                    schema_missing[fld].append(bid)
            for fld in ('evidence_point_id','security_level','dok','audit_status'):
                if fld in item and clean_text(item.get(fld)).strip()=='':
                    schema_blank[fld].append(bid)
            if 'audit_status' in item and clean_text(item.get('audit_status')).strip().upper() not in {'NEEDS AUDIT','KEEP','CLEAN','REBUILD'}:
                hard.append({'where':bid,'issue':f"Invalid audit_status {item.get('audit_status')!r}; allowed values are NEEDS AUDIT, KEEP, CLEAN, REBUILD."})
            if 'supporting_i_can_ids' in item and not isinstance(item.get('supporting_i_can_ids'),list):
                hard.append({'where':bid,'issue':'supporting_i_can_ids must be a list (use [] when none).'})
            if 'tables' in item and not isinstance(item.get('tables'),list):
                hard.append({'where':bid,'issue':'tables must be a list (use [] when none).'})
            if 'figures' in item and not isinstance(item.get('figures'),list):
                hard.append({'where':bid,'issue':'figures must be a list of structured figure objects (use [] when none).'})
            qd=item.get('question_design')
            if 'question_design' in item:
                if not isinstance(qd,dict):
                    hard.append({'where':bid,'issue':'question_design must be an object linking the finished item to its pre-authoring design slot.'})
                else:
                    qd_required={'slot_id','structure_id','wording_profile','response_mode','representation_mode','parallel_family','variation_axes'}
                    missing_qd=[k for k in sorted(qd_required) if k not in qd or (k!='variation_axes' and clean_text(qd.get(k)).strip()=='')]
                    if missing_qd:
                        hard.append({'where':bid,'issue':'question_design is missing required planning field(s): '+', '.join(missing_qd)})
                    if 'variation_axes' in qd and not isinstance(qd.get('variation_axes'),list):
                        hard.append({'where':bid,'issue':'question_design.variation_axes must be a list.'})
            famid=clean_text(item.get('question_family_id')).strip().lower()
            if famid and (re.search(r'(^|[-_])v[1-6]($|[-_])',famid) or re.search(r'(^|[-_])q(?:uestion)?[-_]?\d+($|[-_])',famid)):
                hard.append({'where':bid,'issue':f'question_family_id {item.get("question_family_id")!r} appears to encode form/version/question number. Family IDs must describe the semantic evidence family, not the slot.'})
        for fld,bids in sorted(schema_missing.items()):
            hard.append({'where':'BANK HANDOFF SCHEMA','issue':f'Required literal field {fld!r} is missing from {len(bids)} canonical item(s). Aliases do not count. First IDs: '+', '.join(bids[:8])})
        for fld,bids in sorted(schema_blank.items()):
            hard.append({'where':'BANK HANDOFF SCHEMA','issue':f'Required field {fld!r} is blank on {len(bids)} canonical item(s). First IDs: '+', '.join(bids[:8])})
        counts['handoff_missing_fields']={k:len(v) for k,v in schema_missing.items()}
        counts['handoff_blank_fields']={k:len(v) for k,v in schema_blank.items()}
        linked=[];unlinked=[]
        for item in items:
            qd=item.get('question_design') if isinstance(item.get('question_design'),dict) else {}
            sid=clean_text(qd.get('slot_id')).strip()
            if sid and sid in design_slots: linked.append(str(item.get('bank_id') or '<missing-id>'))
            else: unlinked.append(str(item.get('bank_id') or '<missing-id>'))
        if unlinked:
            hard.append({'where':'QUESTION DESIGN MAP','issue':f'{len(unlinked)} canonical item(s) do not link to a planned slot in unit{unit}_question_design_map.json. First IDs: '+', '.join(unlinked[:10])})
        ck['question_design_map']='PASS' if design_slots and not unlinked else 'FAIL'
        counts['question_design_linked_items']=len(linked)
        ck['machine_handoff_schema']='PASS' if not schema_missing and not schema_blank else 'FAIL'
    try:
    ...
    review_ht=(base/f'unit{unit}_review.html').read_text(encoding='utf-8',errors='replace')
    resource_ht=(base/f'unit{unit}_resources.html').read_text(encoding='utf-8',errors='replace')
    article_re=r'<article\b[^>]*class=["\'][^"\']*\bbank-item\b[^>]*>'
    articles=len(re.findall(article_re,ht,re.I));review_articles=len(re.findall(article_re,review_ht,re.I));resource_articles=len(re.findall(article_re,resource_ht,re.I))
    counts['html_bank_items']=articles;counts['review_html_bank_items']=review_articles;counts['resource_html_bank_items']=resource_articles
    def _resource_seq_item(item):
        dest=clean_text(item.get('destination') or item.get('destination_group')).lower()
        return ('wtc' in dest or "what's to come" in dest or 'what to come' in dest or ('note' in dest and ('example' in dest or 'yti' in dest or 'you try' in dest)) or 'warm' in dest or 'exit' in dest or 'summative' in dest)
    expected_resource=sum(1 for item in items if _resource_seq_item(item))
    if articles==review_articles==len(mapping)==len(items) and resource_articles==expected_resource:
        ck['html_mapping_json']=f'PASS ({articles} canonical; {resource_articles} resource-view)'
    else:
        ck['html_mapping_json']='FAIL';hard.append({'where':'PACKAGE','issue':f'Canonical/view counts disagree: teaching={articles}, review={review_articles}, resources={resource_articles} expected_resources={expected_resource}, mapping={len(mapping)}, JSON={len(items)}'})
    required_resource_anchors=['#wtc','#ex-yti','#warmups','#exit-tickets','#summative-assessments']
    anchor_positions=[resource_ht.find(a) for a in required_resource_anchors]
    if any(pos<0 for pos in anchor_positions):
        hard.append({'where':'RESOURCE VIEW','issue':'unit resource view is missing one or more required top anchors: WTC, EX/YTI, Warm-Ups, Exit Tickets, Summative Assessments.'})
    elif anchor_positions!=sorted(anchor_positions):
        hard.append({'where':'RESOURCE VIEW','issue':'Required resource-view top links are not ordered WTC -> EX/YTI -> Warm-Ups -> Exit Tickets -> Summative Assessments.'})
    if f'unit{unit}_resources.html' not in ht:
        hard.append({'where':'HTML NAV','issue':'Teaching-order Bank view is missing the Unit Resource View link.'})
    for label,page,expected in [('teaching',ht,articles),('review',review_ht,review_articles),('resources',resource_ht,resource_articles)]:
        badge_count=len(re.findall(r'class=["\'][^"\']*\baudit-status\b',page,re.I))
        ican_count=len(re.findall(r'class=["\'][^"\']*\bmeta-ican-text\b',page,re.I))
        data_status_count=len(re.findall(r'data-audit-status=["\'][^"\']+["\']',page,re.I))
        if badge_count!=expected:
            hard.append({'where':f'{label.upper()} HTML','issue':f'Audit-status badge count {badge_count} does not match bank-item count {expected}.'})
        if ican_count!=expected:
            hard.append({'where':f'{label.upper()} HTML','issue':f'Exact I-can metadata count {ican_count} does not match bank-item count {expected}.'})
        if data_status_count!=expected:
            hard.append({'where':f'{label.upper()} HTML','issue':f'data-audit-status count {data_status_count} does not match bank-item count {expected}.'})
    ...
    ids=set();exact=defaultdict(list);numeric_shells=defaultdict(list);semantic_shells=defaultdict(list);practice_tails=defaultdict(list);practice_iteration=[];figure_qa_missing=[];figure_sensitive_missing=[];contradictions=[];context_oddities=[];grammar_oddities=[];source_basis_counts=defaultdict(list)
    for item in items:
        bid=str(item.get('bank_id') or '<missing-id>')
        ...
        if family=='science':
            sh=_physics_semantic_shell(prompt)
            if sh: semantic_shells[sh].append(item)
            if any(rx.search(prompt) for rx in PHYSICS_HIGH_CONFIDENCE_CONTRADICTIONS): contradictions.append(bid)
            if PHYSICS_CONTEXT_ODDITY_RE.search(prompt): context_oddities.append(bid)
            if BAD_ARTICLE_RE.search(prompt): grammar_oddities.append(bid)
            sb=_source_basis_key(item)
            if sb: source_basis_counts[sb].append(bid)
            if re.search(r'<svg\b',str(item.get('student_html') or ''),re.I):
                inline_meta=[f for f in (item.get('figures') or []) if isinstance(f,dict) and clean_text(f.get('render_mode') or f.get('type')).strip().lower() in {'inline_svg','inline-svg'}]
                if not inline_meta:
                    hard.append({'where':bid,'issue':'Inline SVG representation lacks a structured figures[] QA record with render_mode=inline_svg. Inline SVG is not exempt from answer-neutrality verification.'})
        for fig in item.get('figures') or []:
            if family=='science' and not isinstance(fig,dict):
                figure_qa_missing.append((bid,'bare filename/string figure reference'))
                f=str(fig)
            else:
                f=fig.get('file') if isinstance(fig,dict) else str(fig)
                if family=='science' and isinstance(fig,dict):
                    render_mode=clean_text(fig.get('render_mode') or fig.get('type')).strip().lower()
                    if not f and render_mode not in {'inline_svg','inline-svg'}:
                        figure_qa_missing.append((bid,'structured figure missing file/render_mode=inline_svg'))
                    if fig.get('answer_neutrality_verified') is not True or not clean_text(fig.get('student_evidence_role')).strip():
                        figure_qa_missing.append((bid,f or '<missing-file>'))
                    if FIGURE_ANSWER_SENSITIVE_RE.search(prompt) and fig.get('answer_bearing_elements_removed') is not True:
                        figure_sensitive_missing.append((bid,f or '<missing-file>'))
                    if fig.get('teacher_key_only') is True:
                        hard.append({'where':bid,'issue':f'Teacher-key-only figure referenced by student item: {f}'})
            if f and not (base/f).exists():hard.append({'where':bid,'issue':f'Missing referenced figure: {f}'})
        exact[normalize_prompt(prompt)].append(item)
        if 'practice' in dest:
            numeric_shells[_numeric_shell(prompt)].append(bid)
    ...
    dups=[vals for p,vals in exact.items() if p and len(vals)>1];counts['exact_duplicate_groups']=len(dups)
    dup_hard_groups=0
    secure_dup_groups=0
    for vals in dups:
        bids=[str(x.get('bank_id') or '<missing-id>') for x in vals]
        secure=[x for x in vals if _is_secure(x)]
        if secure:
            secure_dup_groups+=1;dup_hard_groups+=1
            hard.append({'where':', '.join(bids[:10]),'issue':f'Exact prompt duplicated across {len(vals)} canonical item(s) and includes secure/Summative evidence. Secure exact duplicates are forbidden within/across forms and against instruction.'})
            continue
        missing_reason=[str(x.get('bank_id') or '<missing-id>') for x in vals[1:] if not _intentional_exact_repeat_reason(x)]
        if missing_reason:
            dup_hard_groups+=1
            hard.append({'where':', '.join(bids[:10]),'issue':f'Exact student prompt duplicated across {len(vals)} canonical instructional items. Reuse the family, not the finished question. Non-secure exact repetition requires explicit intentional_exact_repeat_reason on repeated items.'})
        else:
            review.append({'where':', '.join(bids[:10]),'issue':f'Exact instructional repeat explicitly justified across {len(vals)} items; teacher review the recorded intentional repeat reason.'})
    counts['exact_duplicate_hard_groups']=dup_hard_groups
    counts['secure_exact_duplicate_groups']=secure_dup_groups
    if family=='science':
        sem_groups=[vals for k,vals in semantic_shells.items() if k and len(vals)>=4]
        sem_groups.sort(key=len,reverse=True)
        counts['physics_semantic_shell_groups']=len(sem_groups)
        sem_hard=0
        for vals in sem_groups:
            secure=[x for x in vals if _is_secure(x)]
            instructional=[x for x in vals if not _is_secure(x)]
            bids=[str(x.get('bank_id') or '<missing-id>') for x in vals]
            if len(secure)>=2 or (secure and instructional):
                sem_hard+=1
                hard.append({'where':'SEMANTIC CLONE','issue':f'{len(vals)} canonical items share one number/object/unit-normalized Physics shell and include secure evidence. Number/object swaps do not establish secure independence. First IDs: '+', '.join(bids[:10])})
            elif len(vals)>=8:
                sem_hard+=1
                hard.append({'where':'SEMANTIC CLONE','issue':f'{len(vals)} instructional items share one number/object/unit-normalized Physics shell. Heavy prompt-shell cloning is not purposeful family repetition. First IDs: '+', '.join(bids[:10])})
            else:
                review.append({'where':'SEMANTIC CLONE','issue':f'{len(vals)} instructional items share one normalized Physics shell. Verify meaningful evidence variation. IDs: '+', '.join(bids[:8])})
        counts['physics_semantic_shell_hard_groups']=sem_hard
        if contradictions:
            hard.append({'where':'PHYSICS PLAUSIBILITY','issue':f'{len(contradictions)} item(s) contain a high-confidence contradictory motion state (for example steady motion and at rest). First IDs: '+', '.join(contradictions[:10])})
        if context_oddities:
            review.append({'where':'PHYSICS PLAUSIBILITY','issue':f'{len(context_oddities)} item(s) contain a suspicious setup such as a hanging object resting on a floor/table. Inspect context fit. First IDs: '+', '.join(context_oddities[:10])})
        if grammar_oddities:
            review.append({'where':'PHYSICS LANGUAGE','issue':f'{len(grammar_oddities)} item(s) contain likely a/an noun-substitution grammar residue. First IDs: '+', '.join(grammar_oddities[:10])})
        if source_basis_counts:
            top_basis,top_ids=max(source_basis_counts.items(),key=lambda kv:len(kv[1]))
            frac=len(top_ids)/max(1,len(items))
            counts['dominant_source_basis_fraction']=round(frac,3)
            if len(items)>=100 and frac>=0.80 and any(k in top_basis for k in ('profile','framework','question-family','question family','pm')):
                review.append({'where':'SOURCE PROVENANCE','issue':f'One boilerplate source_basis appears on {len(top_ids)}/{len(items)} items ({frac*100:.0f}%). Record item-level provenance; Framework/PM/profile names should not be stamped mechanically on the whole Bank.'})
    if family=='science' and figure_qa_missing:
        hard.append({'where':'FIGURE QA','issue':f'{len(figure_qa_missing)} student figure reference(s) lack required answer-neutrality verification metadata (file + student_evidence_role + answer_neutrality_verified=true). First: '+', '.join(f'{b}:{f}' for b,f in figure_qa_missing[:8])})
    if family=='science' and figure_sensitive_missing:
        hard.append({'where':'FIGURE QA','issue':f'{len(figure_sensitive_missing)} answer-sensitive figure task(s) lack answer_bearing_elements_removed=true. NEVER SHOW THE ANSWER IN THE STUDENT IMAGE. First: '+', '.join(f'{b}:{f}' for b,f in figure_sensitive_missing[:8])})
    shell_groups=[(k,v) for k,v in numeric_shells.items() if k and len(v)>=4]
    shell_groups.sort(key=lambda kv:len(kv[1]),reverse=True)
    counts['practice_numeric_shell_groups']=len(shell_groups)
    for _,bids in shell_groups[:12]:
        review.append({'where':'PRACTICE','issue':f'{len(bids)} Practice prompts share the same number/unit-normalized shell. Check that this is purposeful family repetition rather than number-only cloning. First IDs: '+', '.join(bids[:8])})
    ...
    for r in mapping:
        ...
        if key:by[sec][key]+=1
    expected={'WTC':1,'NOTES':(8 if family=='science' else 12),'P1':24,'P2':24,'P3':24,'EXTRA':24,'CYU':6,'WU1':4,'WU2':4,'WU3':4,'ETA':4,'ETB':4,'ETC':4};inv=True
    ...
    # WTC FRQ-decomposition contract. WTC is a coherent multipart practice routine and is coverage-exempt.
    if family=='science':
        wtc_items=[]
        for item in items:
            dest=str(item.get('destination') or '').lower()
            if 'wtc' in dest or "what's to come" in dest or 'what to come' in dest:
                wtc_items.append(item)
        counts['wtc_items']=len(wtc_items)
        cross_unit_wtc=0
        for item in wtc_items:
            bid=str(item.get('bank_id') or '<missing-id>')
            if item.get('wtc_frq') is not True:
                hard.append({'where':bid,'issue':'WTC must carry wtc_frq=true and be authored as a true multi-part FRQ.'})
            if item.get('wtc_shared_stimulus') is not True:
                hard.append({'where':bid,'issue':'WTC must carry wtc_shared_stimulus=true; disconnected preview questions are not a WTC.'})
            try:
                pc=int(item.get('wtc_part_count'))
            except Exception:
                pc=0
            if pc<3 or pc>5:
                hard.append({'where':bid,'issue':f'WTC wtc_part_count={pc}; Physics WTC requires 3-5 connected labeled FRQ parts.'})
            focus=item.get('wtc_frq_focus')
            if not isinstance(focus,list) or not [x for x in focus if str(x).strip()]:
                hard.append({'where':bid,'issue':'WTC must carry a non-empty wtc_frq_focus list describing the FRQ breakdown moves practiced.'})
            if str(item.get('coverage_role') or '').strip()!='wtc_frq_practice':
                hard.append({'where':bid,'issue':'WTC coverage_role must be wtc_frq_practice; WTC cannot serve current-Unit coverage.'})
            ic=str(item.get('i_can_id') or '').strip()
            if str(unit).isdigit() and ic and not ic.startswith(f'U{unit}-'):
                cross_unit_wtc+=1
        counts['cross_unit_wtc_items']=cross_unit_wtc
        ck['wtc_frq_contract']='PASS' if not any(h.get('where')==str(i.get('bank_id') or '<missing-id>') and 'WTC' in h.get('issue','') for i in wtc_items for h in hard) else 'FAIL'

    # Physics I-can-first coverage audit. The exact I-can is the atomic evidence address.
    if family=='science':
        try:
            with (base/f'unit{unit}_i_can_map.csv').open(encoding='utf-8-sig',newline='') as f:
                i_can_rows=list(csv.DictReader(f))
        except Exception as e:
            i_can_rows=[];hard.append({'where':'I-CAN MAP','issue':f'Cannot parse unit{unit}_i_can_map.csv: {e}'})
        expected_i_cans={str(r.get('i_can_id') or '').strip():r for r in i_can_rows if str(r.get('i_can_id') or '').strip()}
        counts['i_can_count']=len(expected_i_cans)
        required_ic_columns={'section','title','learning_target','i_can_id','i_can','mastery_goal_id','evidence_job','natural_dok_range','likely_misconceptions','preferred_question_families','representations','notes_anchor_plan','practice_weighting_rationale','secure_assessment_plan','source_basis'}
        found_ic_columns=set(i_can_rows[0].keys()) if i_can_rows else set()
        missing_ic_columns=sorted(required_ic_columns-found_ic_columns)
        if missing_ic_columns:
            hard.append({'where':'I-CAN MAP','issue':'Missing required I-can evidence-plan column(s): '+', '.join(missing_ic_columns)})
        course_name=str(data.get('course') or '').lower()
        if 'physics' in course_name and str(unit).isdigit():
            canonical_ic_ids={f'U{unit}-S{s}-IC{i}' for s in range(1,6) for i in range(1,4)}
            missing_ids=sorted(canonical_ic_ids-set(expected_i_cans))
            extra_ids=sorted(set(expected_i_cans)-canonical_ic_ids)
            if missing_ids:hard.append({'where':'I-CAN MAP','issue':'Missing exact Assessment Plan I-can ID(s): '+', '.join(missing_ids)})
            if extra_ids:hard.append({'where':'I-CAN MAP','issue':'Unexpected I-can ID(s) for this Unit: '+', '.join(extra_ids)})
        if not expected_i_cans:
            hard.append({'where':'I-CAN MAP','issue':'No exact I-can rows found; Physics Bank must provide one row per Assessment Plan I-can.'})
        # Every canonical item must carry one primary i_can_id.
        missing_primary=[]
        for item in items:
            bid=str(item.get('bank_id') or '<missing-id>')
            if not str(item.get('i_can_id') or '').strip():missing_primary.append(bid)
        if missing_primary:
            hard.append({'where':'I-CAN COVERAGE','issue':f'{len(missing_primary)} canonical item(s) lack a primary i_can_id. First IDs: {", ".join(missing_primary[:8])}'})

        by_ic=defaultdict(lambda:defaultdict(int))
        for r in mapping:
            ic=str(r.get('i_can_id') or '').strip();dest=str(r.get('destination') or r.get('destination_group') or '').lower()
            if not ic:continue
            if 'note' in dest and 'example' in dest:by_ic[ic]['NOTE_EX']+=1
            elif 'note' in dest and ('yti' in dest or 'you try' in dest):by_ic[ic]['NOTE_YTI']+=1
            elif 'practice' in dest and 'extra' in dest:by_ic[ic]['EXTRA']+=1
            elif 'practice' in dest and ('set 1' in dest or 'practice 1' in dest or 'ps1' in dest):by_ic[ic]['P1']+=1
            elif 'practice' in dest and ('set 2' in dest or 'practice 2' in dest or 'ps2' in dest):by_ic[ic]['P2']+=1
            elif 'practice' in dest and ('set 3' in dest or 'practice 3' in dest or 'ps3' in dest):by_ic[ic]['P3']+=1
            elif 'cyu' in dest:by_ic[ic]['CYU']+=1
            elif 'warm' in dest:by_ic[ic]['WU']+=1
            elif 'exit' in dest and ('et-a' in dest or 'ticket a' in dest or 'ticket 1' in dest):by_ic[ic]['ETA']+=1
            elif 'exit' in dest and ('et-b' in dest or 'ticket b' in dest or 'ticket 2' in dest):by_ic[ic]['ETB']+=1
            elif 'exit' in dest and ('et-c' in dest or 'ticket c' in dest or 'ticket 3' in dest):by_ic[ic]['ETC']+=1
        counts['i_can_destination_coverage']={ic:dict(vals) for ic,vals in sorted(by_ic.items())}
        ic_ok=True
        for ic in sorted(expected_i_cans):
            vals=by_ic.get(ic,{})
            floors={'NOTE_EX':1,'NOTE_YTI':1,'P1':1,'P2':1,'P3':1,'EXTRA':1,'CYU':1,'WU':1,'ETA':1,'ETB':1,'ETC':1}
            for key,n in floors.items():
                if vals.get(key,0)<n:
                    ic_ok=False;hard.append({'where':ic,'issue':f'I-can-first coverage {key}={vals.get(key,0)}, minimum {n}.'})
        # I-can-first must not become automatic arithmetic symmetry (e.g., 24/3 = 8/8/8).
        # Surface sections where all four Practice containers use the same perfectly equal split.
        by_sec_ic=defaultdict(lambda:defaultdict(lambda:defaultdict(int)))
        ic_to_sec={ic:str(row.get('section') or '').strip() for ic,row in expected_i_cans.items()}
        for r in mapping:
            ic=str(r.get('i_can_id') or '').strip();dest=str(r.get('destination') or r.get('destination_group') or '').lower()
            sec=ic_to_sec.get(ic,'')
            if not ic or not sec:continue
            key=None
            if 'practice' in dest and 'extra' in dest:key='EXTRA'
            elif 'practice' in dest and ('set 1' in dest or 'practice 1' in dest or 'ps1' in dest):key='P1'
            elif 'practice' in dest and ('set 2' in dest or 'practice 2' in dest or 'ps2' in dest):key='P2'
            elif 'practice' in dest and ('set 3' in dest or 'practice 3' in dest or 'ps3' in dest):key='P3'
            if key:by_sec_ic[sec][key][ic]+=1
        equal_practice_sections=[]
        for sec,dests in sorted(by_sec_ic.items()):
            if all(k in dests for k in ('P1','P2','P3','EXTRA')):
                splits=[]
                for k in ('P1','P2','P3','EXTRA'):
                    vals=list(dests[k].values())
                    splits.append(vals)
                if splits and all(len(v)==3 and len(set(v))==1 for v in splits) and all(sorted(v)==sorted(splits[0]) for v in splits[1:]):
                    equal_practice_sections.append((sec,splits[0][0] if splits[0] else 0))
        counts['mechanically_equal_practice_sections']=len(equal_practice_sections)
        if equal_practice_sections:
            review.append({'where':'I-CAN ALLOCATION','issue':'Practice allocation is perfectly equal across P1/P2/P3/Extra in section(s) '+', '.join(f'{s} ({n}/{n}/{n})' for s,n in equal_practice_sections)+'. I-can-first is not a quota system; verify the evidence plan independently justifies equal weighting rather than defaulting to 24/3.'})
        # Detect a fixed four-container weighting template copied across multiple sections even when it is not 8/8/8.
        signature_to_sections=defaultdict(list)
        for sec,dests in sorted(by_sec_ic.items()):
            ics=sorted([ic for ic,s0 in ic_to_sec.items() if s0==sec])
            if len(ics)!=3 or not all(k in dests for k in ('P1','P2','P3','EXTRA')): continue
            sig=tuple(tuple(dests[k].get(ic,0) for ic in ics) for k in ('P1','P2','P3','EXTRA'))
            signature_to_sections[sig].append(sec)
        repeated_alloc=[(sig,secs) for sig,secs in signature_to_sections.items() if len(secs)>=3]
        counts['repeated_practice_allocation_signatures']=len(repeated_alloc)
        for sig,secs in repeated_alloc:
            hard.append({'where':'I-CAN ALLOCATION','issue':f'The same P1/P2/P3/Extra I-can weighting signature {sig} is reused across {len(secs)} sections ({", ".join(secs)}). This is a fixed quota template, not section-specific evidence planning.'})

        # Representation-explicit I-cans require actual representations, not prose descriptions of a diagram/model.
        rep_explicit={ic for ic,row in expected_i_cans.items() if REPRESENTATION_EXPLICIT_RE.search(str(row.get('i_can') or ''))}
        counts['representation_explicit_i_cans']=sorted(rep_explicit)
        if rep_explicit:
            instr_rep=defaultdict(int)
            summ_rep=defaultdict(lambda:defaultdict(int))
            for item in items:
                ic=str(item.get('i_can_id') or '').strip()
                if ic not in rep_explicit: continue
                v=str(item.get('summative_version') or '').strip().upper()
                if _has_actual_representation(item):
                    if v: summ_rep[v][ic]+=1
                    else: instr_rep[ic]+=1
            for ic in sorted(rep_explicit):
                if instr_rep.get(ic,0)<1:
                    ic_ok=False;hard.append({'where':ic,'issue':'Representation-explicit I-can has no direct instructional item with an actual student-facing figure/table/vector/diagram. Text describing a representation is not equivalent evidence.'})
                for v in ('V1','V2','V3','V4','V5','V6'):
                    if summ_rep.get(v,{}).get(ic,0)<1:
                        ic_ok=False;hard.append({'where':f'SUMMATIVE {v}','issue':f'Representation-explicit I-can {ic} is directly assessed without an actual student-facing representation on this form.'})
            ck['representation_fidelity']='PASS' if all(instr_rep.get(ic,0)>=1 and all(summ_rep.get(v,{}).get(ic,0)>=1 for v in ('V1','V2','V3','V4','V5','V6')) for ic in rep_explicit) else 'FAIL'
        else:
            ck['representation_fidelity']='PASS'

        # Every secure Summative form must directly sample every exact Unit I-can once as primary coverage.
        summ_by_v=defaultdict(lambda:defaultdict(int))
        for item in items:
            v=str(item.get('summative_version') or '').strip().upper();ic=str(item.get('i_can_id') or '').strip()
            if v and ic:summ_by_v[v][ic]+=1
        counts['i_can_summative_coverage']={v:dict(vals) for v,vals in sorted(summ_by_v.items())}
        for v in ('V1','V2','V3','V4','V5','V6'):
            for ic in sorted(expected_i_cans):
                if summ_by_v.get(v,{}).get(ic,0)<1:
                    ic_ok=False;hard.append({'where':f'SUMMATIVE {v}','issue':f'Missing direct primary coverage for exact I-can {ic}.'})
        ck['i_can_first_coverage']='PASS' if ic_ok and not missing_primary and expected_i_cans else 'FAIL'
...
def write_report(base,status,counts,hard,review,graph_ok,cleanup_ok,final_zip=None):
    ...
    for label,key in [('Required files','required_files'),('HTML / mapping / JSON','html_mapping_json'),('Question design map','question_design_map'),('Machine handoff schema','machine_handoff_schema'),('I-can representation fidelity','representation_fidelity'),('MathJax / HTML','mathjax_html'),('Locked section inventory','locked_inventory'),('Exports','exports'),('Summative profile','summative_profile'),('Question review scan','question_review')]:lines.append(f'- {label}: {ck.get(key,"?")}')
