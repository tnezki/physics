...
# Unit 2.3 notes figure: constant acceleration velocity-time graph
fig, ax = plt.subplots(figsize=(3.5, 3.5))
make_context_graph(
    ax,
    [{'expr': lambda x: 2*x,
      'deriv': lambda x: np.full_like(x, 2.0),
      'color': 'steelblue'}],
    xmin=0, xmax=5, ymin=0, ymax=10,
    xlabel='Time (s)', ylabel='Velocity (m/s)', title=''
)
save_graph(fig, 'u2_3_velocity_time_constant_acceleration.png')
plt.close(fig)
