...
# Unit 1 Section 1 notes graph-generation blocks
...
fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_context_graph(ax, [{'expr': lambda x: 2+3*x, 'deriv': lambda x: 0*x+3, 'color': 'steelblue', 'label': None}], xmin=0, xmax=6, ymin=0, ymax=20, xlabel='Tasks', ylabel='Points', title='')
_mark_point(ax, 4, 14, '(4,14)')
save_graph(fig, 'u1_s1_mn1_input_output_point.png')
plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_context_graph(ax, [{'expr': lambda x: 5+2*x, 'deriv': lambda x: 0*x+2, 'color': 'steelblue', 'label': None}], xmin=0, xmax=6, ymin=0, ymax=20, xlabel='Plays', ylabel='Cost', title='')
_mark_point(ax, 3, 11, '(3,11)')
save_graph(fig, 'u1_s1_mn2_table_equation_graph.png')
plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_context_graph(ax, [{'expr': lambda x: 7+3*x, 'deriv': lambda x: 0*x+3, 'color': 'steelblue', 'label': None}], xmin=0, xmax=5, ymin=0, ymax=25, xlabel='Step', ylabel='Total', title='')
_mark_point(ax, 0, 7, 'start')
_mark_point(ax, 3, 16, '+3 each')
save_graph(fig, 'u1_s1_mn4_start_rate.png')
plt.close(fig)
