...
# Unit 7 Section 7.3 — voltage-current representation
fig, ax = plt.subplots(figsize=(3.5, 3.5))
functions = [
    {
        'expr': lambda x: 3*x,
        'deriv': lambda x: np.full_like(x, 3.0, dtype=float),
        'color': 'steelblue',
        'label': None,
    }
]
make_context_graph(
    ...
    xlabel='Current (A)',
    ylabel='Voltage (V)',
    title='Voltage vs. Current for a Resistor',
)
save_graph(fig, 'u7_3_voltage_current_graph.png')
plt.close(fig)
