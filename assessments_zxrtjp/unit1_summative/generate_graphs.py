...
# Auto-generated Unit 1 summative graph blocks appended below the required marker.

# Unit 1 summative graph-generation blocks
# All figures use graph_tool.py functions. Styling is not overridden.
# Literal filename audit: u1_summ_relation_v1.png u1_summ_relation_v2.png u1_summ_relation_v3.png u1_summ_relation_v4.png u1_summ_growth_v7.png u1_summ_growth_v8.png u1_summ_context_compare_v7.png u1_summ_context_compare_v8.png u1_summ_rect_mismatch_v7.png u1_summ_rect_mismatch_v8.png u1_summ_blank_coordinate.png

fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_standard_graph(ax, [], title='')
save_graph(fig, 'u1_summ_blank_coordinate.png')
plt.close(fig)

_relation_points = {
    1: ([0, 1, 1, 2, 3], [3, 4, 6, 7, 8]),
    2: ([0, 2, 2, 3, 4], [2, 5, 9, 8, 10]),
    3: ([-1, 0, 2, 2, 3], [4, 5, 7, 10, 11]),
    4: ([1, 1, 2, 3, 4], [2, 5, 7, 9, 11]),
}
for _v, (_xs, _ys) in _relation_points.items():
    fig, ax = plt.subplots(figsize=(3.5, 3.5))
    fig.patch.set_facecolor('white')
    make_scatter_plot(ax, _xs, _ys, xmin=-1, xmax=5, ymin=0, ymax=12,
                      color='steelblue', point_size=44, line_of_best_fit=False,
                      xlabel='Input', ylabel='Output', title='')
    save_graph(fig, f'u1_summ_relation_v{_v}.png')
    plt.close(fig)

_growth_defs = {
    7: (lambda x: 3*(1.4**x), lambda x: 3*np.log(1.4)*(1.4**x), 0, 8, 0, 45),
    8: (lambda x: 2*(1.55**x), lambda x: 2*np.log(1.55)*(1.55**x), 0, 7, 0, 45),
}
for _v, (_expr, _deriv, _xmin, _xmax, _ymin, _ymax) in _growth_defs.items():
    fig, ax = plt.subplots(figsize=(3.5, 3.5))
    fig.patch.set_facecolor('white')
    make_context_graph(ax, [
        {'expr': _expr, 'deriv': _deriv, 'color': 'steelblue', 'label': None}
    ], xmin=_xmin, xmax=_xmax, ymin=_ymin, ymax=_ymax,
       xlabel='Step', ylabel='Output', title='')
    save_graph(fig, f'u1_summ_growth_v{_v}.png')
    plt.close(fig)

_compare_defs = {
    7: (lambda x: 2*x + 1, lambda x: 0*x + 2, 0, 6, 0, 18),
    8: (lambda x: 3*x + 2, lambda x: 0*x + 3, 0, 6, 0, 24),
}
for _v, (_expr, _deriv, _xmin, _xmax, _ymin, _ymax) in _compare_defs.items():
    fig, ax = plt.subplots(figsize=(3.5, 3.5))
    fig.patch.set_facecolor('white')
    make_context_graph(ax, [
        {'expr': _expr, 'deriv': _deriv, 'color': 'steelblue', 'label': None}
    ], xmin=_xmin, xmax=_xmax, ymin=_ymin, ymax=_ymax,
       xlabel='Time', ylabel='Distance', title='')
    save_graph(fig, f'u1_summ_context_compare_v{_v}.png')
    plt.close(fig)

make_rectangle_model([r'$x$', r'$2$'], [r'$x$', r'$5$'],
                     [[r'$x^2$', r'$5x$'], [r'$2x$', r'$8$']],
                     'u1_summ_rect_mismatch_v7.png')
make_rectangle_model([r'$x$', r'$3$'], [r'$x$', r'$4$'],
                     [[r'$x^2$', r'$4x$'], [r'$3x$', r'$10$']],
                     'u1_summ_rect_mismatch_v8.png')
