...
# =============================================================================
# UNIT 7 FORMATIVE GRAPH-GENERATION BLOCKS — appended after authoritative source
# =============================================================================

def _u7_save_context(filename, functions, xmax=4, ymax=20, title='Voltage–Current Data'):
    fig, ax = plt.subplots(figsize=(3.5, 3.5))
    make_context_graph(ax, functions, xmin=0, xmax=xmax, ymin=0, ymax=ymax,
                       xlabel='Current (A)', ylabel='Voltage (V)', title=title)
    save_graph(fig, filename)
    plt.close(fig)

_u7_save_context('u07_dok3_3_v1.png', [
    {'expr':lambda x:4*x,'deriv':lambda x:np.zeros_like(x)+4,'color':'steelblue','label':None}
], xmax=4, ymax=16)

_u7_save_context('u07_dok3_3_v2.png', [
    {'expr':lambda x:8*x,'deriv':lambda x:np.zeros_like(x)+8,'color':'steelblue','label':None}
], xmax=2.5, ymax=20)

_u7_save_context('u07_dok3_3_v3.png', [
    {'expr':lambda x:3*x,'deriv':lambda x:np.zeros_like(x)+3,'color':'steelblue','label':'Device A'},
    {'expr':lambda x:6*x,'deriv':lambda x:np.zeros_like(x)+6,'color':'firebrick','label':'Device B'}
], xmax=3, ymax=18)

_u7_save_context('u07_dok3_3_v4.png', [
    {'expr':lambda x:2*x+0.5*x**2,'deriv':lambda x:2+x,'color':'steelblue','label':None}
], xmax=4, ymax=16)

_u7_save_context('u07_dok3_3_v5.png', [
    {'expr':lambda x:5*x,'deriv':lambda x:np.zeros_like(x)+5,'color':'steelblue','label':None}
], xmax=4, ymax=20)

_u7_save_context('u07_dok3_3_v6.png', [
    {'expr':lambda x:1.5*x+0.7*x**2,'deriv':lambda x:1.5+1.4*x,'color':'steelblue','label':None}
], xmax=4, ymax=18)

_u7_save_context('u07_dok3_3_v7.png', [
    {'expr':lambda x:2*x,'deriv':lambda x:np.zeros_like(x)+2,'color':'steelblue','label':'Device A'},
    {'expr':lambda x:5*x,'deriv':lambda x:np.zeros_like(x)+5,'color':'firebrick','label':'Device B'}
], xmax=4, ymax=20)

_u7_save_context('u07_dok3_3_v8.png', [
    {'expr':lambda x:4*x,'deriv':lambda x:np.zeros_like(x)+4,'color':'steelblue','label':None}
], xmax=4, ymax=16)
