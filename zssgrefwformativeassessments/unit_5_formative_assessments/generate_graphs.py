...
# ============================================================================
# UNIT 5 FORMATIVE GRAPH GENERATION -- appended after authoritative source
# ============================================================================

# DOK 3.4: resonance response curve / evidence graph
fig, ax = plt.subplots(figsize=(3.5, 3.5))
make_scatter_plot(
    ax,
    x_data=[2,3,4,5,5.5,6,6.5,7,8,9,10],
    y_data=[1.0,1.2,1.6,3.2,5.8,8.9,5.9,3.4,1.8,1.3,1.0],
    xmin=1, xmax=11, ymin=0, ymax=10,
    xlabel='Driving frequency (Hz)', ylabel='Response amplitude (arb. units)',
    title='Response to Driving Frequency'
)
save_graph(fig, 'u5_f_dok3_4_resonance_scan_sq_v1.png')
plt.close(fig)

# DOK 4.3: experimental resonance frequency-sweep data
fig, ax = plt.subplots(figsize=(3.5, 3.5))
make_scatter_plot(
    ax,
    x_data=[2.5,3,3.5,4,4.5,5,5.5,6,6.5,7,7.5,8,8.5,9],
    y_data=[1.0,1.1,1.2,1.4,1.9,2.8,5.1,8.4,5.4,3.0,2.0,1.5,1.2,1.1],
    xmin=2, xmax=9.5, ymin=0, ymax=9,
    xlabel='Driving frequency (Hz)', ylabel='Measured response (cm)',
    title='Frequency-Sweep Measurements'
)
save_graph(fig, 'u5_f_dok4_3_resonance_data_sq_v1.png')
plt.close(fig)

# DOK 4.4: beat loudness pattern, shown as relative loudness rather than source waveform
fig, ax = plt.subplots(figsize=(3.5, 3.5))
make_context_graph(
    ax,
    functions=[{
        'expr': lambda x: 2.1 + 1.7*np.cos(2*np.pi*4*x),
        'deriv': lambda x: -1.7*2*np.pi*4*np.sin(2*np.pi*4*x),
        'color': 'steelblue',
        'label': None
    }],
    xmin=0, xmax=1.0, ymin=0, ymax=4,
    xlabel='Time (s)', ylabel='Relative loudness',
    title='Combined Loudness Over Time'
)
save_graph(fig, 'u5_f_dok4_4_beats_pattern_sq_v1.png')
plt.close(fig)
