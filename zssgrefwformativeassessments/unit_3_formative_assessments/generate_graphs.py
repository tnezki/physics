...
# =============================================================================
# UNIT 3 FORMATIVE GRAPH GENERATION CALLS — appended after authoritative source
# =============================================================================

def _u3_context(filename, fn, deriv, xmin, xmax, ymin, ymax, xlabel, ylabel):
    fig, ax = plt.subplots(figsize=(5.2, 3.0))
    fig.patch.set_facecolor('white')
    make_context_graph(ax, [dict(expr=fn, deriv=deriv, color='steelblue', label=None)],
                       xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax,
                       xlabel=xlabel, ylabel=ylabel, title='')
    save_graph(fig, filename)
    plt.close(fig)

# Terminal-speed evidence graphs: speed approaches a constant value.
_u3_context('u3_dok3_3_v3.png', lambda t: 48*(1-np.exp(-0.55*t)), lambda t: 26.4*np.exp(-0.55*t), 0, 10, 0, 55, 'Time (s)', 'Speed (m/s)')
_u3_context('u3_dok3_3_v5.png', lambda t: 42*(1-np.exp(-0.62*t)), lambda t: 26.04*np.exp(-0.62*t), 0, 9, 0, 50, 'Time (s)', 'Speed (m/s)')
_u3_context('u3_dok3_3_v8.png', lambda t: 55*(1-np.exp(-0.48*t)), lambda t: 26.4*np.exp(-0.48*t), 0, 12, 0, 62, 'Time (s)', 'Speed (m/s)')

# Acceleration versus net-force evidence for one constant-mass cart.
_u3_context('u3_dok4_1_v5.png', lambda F: 0.20*F, lambda F: np.full_like(F,0.20), 0, 40, 0, 9, 'Net force (N)', 'Acceleration (m/s^2)')
_u3_context('u3_dok4_1_v6.png', lambda F: 0.25*F, lambda F: np.full_like(F,0.25), 0, 32, 0, 9, 'Net force (N)', 'Acceleration (m/s^2)')

# Additional falling-object evidence graphs for DOK 4 critique.
_u3_context('u3_dok4_2_v5.png', lambda t: 50*(1-np.exp(-0.50*t)), lambda t: 25*np.exp(-0.50*t), 0, 11, 0, 57, 'Time (s)', 'Speed (m/s)')
_u3_context('u3_dok4_2_v6.png', lambda t: 46*(1-np.exp(-0.58*t)), lambda t: 26.68*np.exp(-0.58*t), 0, 10, 0, 53, 'Time (s)', 'Speed (m/s)')
