...
# UNIT 7 EXIT TICKET GRAPH GENERATION
if __name__ == '__main__':
    # Section 7.3 Ticket B: constant-resistance V-I graph.
    fig, ax = plt.subplots(figsize=(3.5, 3.5))
    make_context_graph(
        ax,
        functions=[{
            'expr': lambda x: 4*x,
            'deriv': lambda x: np.full_like(x, 4.0),
            'color': 'steelblue',
            'label': None,
        }],
    ...
    save_graph(fig, 'u7_3_b_vi_line.png')
    plt.close(fig)

    # Section 7.3 Ticket C: nonconstant-resistance V-I graph.
    fig, ax = plt.subplots(figsize=(3.5, 3.5))
    make_context_graph(
        ax,
        functions=[{
            'expr': lambda x: 2*x + x**2,
            'deriv': lambda x: 2 + 2*x,
            'color': 'steelblue',
            'label': None,
        }],
        xmin=0, xmax=4, ymin=0, ymax=24,
        xlabel='Current (A)', ylabel='Voltage (V)',
        title='Mystery Device'
    )
    save_graph(fig, 'u7_3_c_vi_curve.png')
    plt.close(fig)

    # Section 7.4 Ticket B: daily energy-use comparison.
    fig, ax = plt.subplots(figsize=(4.0, 3.0))
    make_bar_chart(
        ax,
        categories=['Laptop', 'Fan', 'Lamp', 'Router'],
        values=[260, 280, 120, 288],
        ylabel='Daily energy (Wh)',
        xlabel='Device',
        title='Daily Electrical Energy Use'
    )
    save_graph(fig, 'u7_4_b_energy_bar.png')
    plt.close(fig)
