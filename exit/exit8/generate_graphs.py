...
# =============================================================================
# UNIT 8 EXIT-TICKET GRAPH GENERATION BLOCKS
# Appended below the authoritative source without modifying any line above.
# =============================================================================

# Section 8.4 — AC generator voltage-time graph, moderate rotation rate.
fig, ax = plt.subplots(figsize=(3.5, 3.5))
make_context_graph(
    ax,
    functions=[{
        'expr': lambda t: 12*np.sin(np.pi*t),
        'deriv': lambda t: 12*np.pi*np.cos(np.pi*t),
        'color': 'steelblue',
        'label': None,
    }],
    xmin=0, xmax=4, ymin=-15, ymax=15,
    xlabel='Time (s)', ylabel='Voltage (V)',
    title='Generator Output'
)
save_graph(fig, 'u8_4_b_ac_voltage.png')
plt.close(fig)

# Section 8.4 — AC generator voltage-time graph, faster rotation rate.
fig, ax = plt.subplots(figsize=(3.5, 3.5))
make_context_graph(
    ax,
    functions=[{
        'expr': lambda t: 8*np.sin(2*np.pi*t),
        'deriv': lambda t: 16*np.pi*np.cos(2*np.pi*t),
        'color': 'steelblue',
        'label': None,
    }],
...
save_graph(fig, 'u8_4_c_ac_fast.png')
plt.close(fig)
