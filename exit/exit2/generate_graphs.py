...
# =============================================================================
# UNIT 2 EXIT TICKET GRAPH GENERATION BLOCKS
# =============================================================================

def _unit2_exit_graphs():
    graphs = [
        ('u2_3_a_vt_rising.png', lambda x: 2*x, lambda x: np.full_like(x, 2.0), 0, 4, 0, 10, 'Time (s)', 'Velocity (m/s)', 'Velocity vs. Time'),
        ('u2_3_b_dt_curve.png', lambda x: x**2, lambda x: 2*x, 0, 4, 0, 18, 'Time (s)', 'Distance (m)', 'Distance vs. Time'),
        ('u2_3_c_vt_horizontal.png', lambda x: np.full_like(x, 8.0), lambda x: np.zeros_like(x), 0, 5, 0, 10, 'Time (s)', 'Velocity (m/s)', 'Velocity vs. Time'),
        ('u2_5_b_dt_line.png', lambda x: 4*x, lambda x: np.full_like(x, 4.0), 0, 5, 0, 22, 'Time (s)', 'Distance (m)', 'Distance vs. Time'),
        ('u2_5_c_vt_rising.png', lambda x: 1+2*x, lambda x: np.full_like(x, 2.0), 0, 3, 0, 8, 'Time (s)', 'Velocity (m/s)', 'Velocity vs. Time'),
        ('u2_5_c_dt_curve.png', lambda x: 2*x**2, lambda x: 4*x, 0, 3, 0, 20, 'Time (s)', 'Distance (m)', 'Distance vs. Time'),
    ]
    for filename, expr, deriv, xmin, xmax, ymin, ymax, xlabel, ylabel, title in graphs:
        fig, ax = plt.subplots(figsize=(3.5, 3.5))
        make_context_graph(
            ax,
            [{'expr': expr, 'deriv': deriv, 'color': 'steelblue'}],
