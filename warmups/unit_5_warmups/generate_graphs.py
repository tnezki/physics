...
# =============================================================================
# UNIT 5 WARMUP FIGURES — APPENDED GENERATION BLOCKS ONLY
# =============================================================================

# 1. Transverse wave profile: wavelength is 4 m.
fig, ax = plt.subplots(figsize=(3.5, 3.0))
wave = [{
    'expr': lambda x: 1.5*np.sin(np.pi*x/2),
    'deriv': lambda x: (1.5*np.pi/2)*np.cos(np.pi*x/2),
    'color': 'steelblue',
    'label': None,
}]
make_context_graph(ax, wave, 0, 8, -2, 2,
                   xlabel='Position (m)', ylabel='Displacement (m)', title='')
save_graph(fig, 'u5_wave_profile.png')
plt.close(fig)

# 2. Two waves with different wavelengths in the same position window.
fig, ax = plt.subplots(figsize=(3.5, 3.0))
compare = [
    {
        'expr': lambda x: 1.2*np.sin(np.pi*x/2),
        'deriv': lambda x: (1.2*np.pi/2)*np.cos(np.pi*x/2),
        'color': 'steelblue',
        'label': 'Wave A',
    },
    {
        'expr': lambda x: 0.8*np.sin(np.pi*x),
        'deriv': lambda x: 0.8*np.pi*np.cos(np.pi*x),
        'color': 'firebrick',
        'label': 'Wave B',
    },
]
make_context_graph(ax, compare, 0, 8, -2, 2,
                   xlabel='Position (m)', ylabel='Displacement (m)', title='')
save_graph(fig, 'u5_wave_compare.png')
plt.close(fig)

# 3. Constructive superposition: two in-phase waves and their resultant.
fig, ax = plt.subplots(figsize=(3.5, 3.0))
interference = [
    {
        'expr': lambda x: 0.7*np.sin(np.pi*x/2),
        'deriv': lambda x: (0.7*np.pi/2)*np.cos(np.pi*x/2),
        'color': 'steelblue',
        'label': 'Wave A',
    },
    {
        'expr': lambda x: 0.5*np.sin(np.pi*x/2),
        'deriv': lambda x: (0.5*np.pi/2)*np.cos(np.pi*x/2),
        'color': 'firebrick',
        'label': 'Wave B',
    },
    {
...
make_context_graph(ax, interference, 0, 8, -1.6, 1.6,
                   xlabel='Position', ylabel='Displacement', title='')
save_graph(fig, 'u5_interference.png')
plt.close(fig)

# 4. Sound travel distance versus time at 340 m/s.
fig, ax = plt.subplots(figsize=(3.5, 3.0))
sound_distance = [{
    'expr': lambda t: 340*t,
    'deriv': lambda t: 0*t + 340,
    'color': 'steelblue',
    'label': None,
}]
make_context_graph(ax, sound_distance, 0, 2, 0, 700,
                   xlabel='Time (s)', ylabel='Distance (m)', title='')
save_graph(fig, 'u5_sound_distance.png')
plt.close(fig)

# 5. Echo one-way distance versus round-trip echo time: d = 170t.
fig, ax = plt.subplots(figsize=(3.5, 3.0))
echo = [{
    'expr': lambda t: 170*t,
    'deriv': lambda t: 0*t + 170,
    'color': 'steelblue',
    'label': None,
}]
make_context_graph(ax, echo, 0, 2, 0, 340,
                   xlabel='Echo time (s)', ylabel='One-way distance (m)', title='')
save_graph(fig, 'u5_echo_graph.png')
plt.close(fig)

# 6. Resonance response data.
fig, ax = plt.subplots(figsize=(4.0, 3.0))
make_bar_chart(ax, ['2','4','6','8','10'], [1,2,7,2,1],
               title='', ylabel='Response amplitude', xlabel='Driving frequency (Hz)')
save_graph(fig, 'u5_resonance.png')
plt.close(fig)

# 7. Beat pattern from 5 Hz and 6 Hz waves.
fig, ax = plt.subplots(figsize=(3.5, 3.0))
beats = [{
    'expr': lambda t: np.sin(2*np.pi*5*t) + np.sin(2*np.pi*6*t),
    'deriv': lambda t: 2*np.pi*5*np.cos(2*np.pi*5*t) + 2*np.pi*6*np.cos(2*np.pi*6*t),
    'color': 'steelblue',
    'label': None,
}]
make_context_graph(ax, beats, 0, 2, -2.2, 2.2,
                   xlabel='Time (s)', ylabel='Combined displacement', title='')
save_graph(fig, 'u5_beats.png')
plt.close(fig)
