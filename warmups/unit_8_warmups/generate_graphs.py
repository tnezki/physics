...
# =============================================================================
# UNIT 8 WARMUP FIGURE GENERATION — APPENDED BLOCKS ONLY
# =============================================================================

def _u8_context(filename, expr, deriv, xmin, xmax, ymin, ymax, xlabel, ylabel, label=None, color='steelblue'):
    fig, ax = plt.subplots(figsize=(3.5, 3.5))
    funcs = [{'expr': expr, 'deriv': deriv, 'color': color, 'label': label}]
    make_context_graph(ax, funcs, xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax,
                       xlabel=xlabel, ylabel=ylabel, title='')
    save_graph(fig, filename)
    plt.close(fig)

_u8_context('u8_u7_ohm_retrieval.png',
            lambda x: 6*x, lambda x: np.full_like(x, 6.0),
            0, 4, 0, 24, 'Current (A)', 'Voltage (V)')

_u8_context('u8_u7_power_retrieval.png',
            lambda x: 12*x, lambda x: np.full_like(x, 12.0),
            0, 5, 0, 60, 'Current (A)', 'Power (W)')

_u8_context('u8_u7_charge_time.png',
            lambda x: 2.5*x, lambda x: np.full_like(x, 2.5),
            0, 10, 0, 25, 'Time (s)', 'Charge (C)')

_u8_context('u8_u7_vi_retrieval.png',
            lambda x: 8*x, lambda x: np.full_like(x, 8.0),
            0, 4, 0, 32, 'Current (A)', 'Voltage (V)')

_u8_context('u8_magnetic_force_speed.png',
            lambda x: 6*x, lambda x: np.full_like(x, 6.0),
            0, 5, 0, 30, 'Speed (m/s)', 'Magnetic Force (N)')

_u8_context('u8_magnetic_force_field.png',
            lambda x: 8*x, lambda x: np.full_like(x, 8.0),
            0, 4, 0, 32, 'Magnetic Field (T)', 'Magnetic Force (N)')

_u8_context('u8_magnetic_force_current.png',
            lambda x: 1*x, lambda x: np.full_like(x, 1.0),
            0, 6, 0, 6, 'Current (A)', 'Magnetic Force (N)')

_u8_context('u8_ac_voltage_time.png',
            lambda x: 12*np.sin(4*np.pi*x), lambda x: 48*np.pi*np.cos(4*np.pi*x),
            0, 1.5, -15, 15, 'Time (s)', 'Voltage (V)')

_u8_context('u8_induction_rate.png',
            lambda x: 2*x, lambda x: np.full_like(x, 2.0),
            0, 5, 0, 10, 'Relative Change Rate', 'Relative Induced Voltage')

fig, ax = plt.subplots(figsize=(3.5, 3.5))
funcs = [
    {'expr': lambda x: 8*np.sin(2*np.pi*x),
     'deriv': lambda x: 16*np.pi*np.cos(2*np.pi*x),
     'color': 'steelblue', 'label': 'Generator A'},
    {'expr': lambda x: 8*np.sin(4*np.pi*x),
     'deriv': lambda x: 32*np.pi*np.cos(4*np.pi*x),
     'color': 'firebrick', 'label': 'Generator B'},
]
make_context_graph(ax, funcs, xmin=0, xmax=2, ymin=-10, ymax=10,
                   xlabel='Time (s)', ylabel='Voltage (V)', title='')
save_graph(fig, 'u8_generator_frequency_compare.png')
plt.close(fig)
