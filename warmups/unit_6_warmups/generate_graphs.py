...
# =============================================================================
# UNIT 6 WARMUP FIGURES — APPENDED GENERATION BLOCKS ONLY
# =============================================================================

# 1. Unit 5 retrieval: two time-domain waves with different frequencies.
fig, ax = plt.subplots(figsize=(3.5, 3.0))
wave_retrieval = [
    {
        'expr': lambda t: 1.0*np.sin(2*np.pi*4*t),
        'deriv': lambda t: 2*np.pi*4*np.cos(2*np.pi*4*t),
        'color': 'steelblue',
        'label': 'Wave A',
    },
    {
        'expr': lambda t: 1.0*np.sin(2*np.pi*2*t),
        'deriv': lambda t: 2*np.pi*2*np.cos(2*np.pi*2*t),
        'color': 'firebrick',
        'label': 'Wave B',
    },
]
make_context_graph(ax, wave_retrieval, 0, 1, -1.4, 1.4,
                   xlabel='Time (s)', ylabel='Displacement', title='')
save_graph(fig, 'u6_u5_wave_retrieval.png')
plt.close(fig)

# 2. Material transmission comparison for Section 6.1 retrieval.
fig, ax = plt.subplots(figsize=(4.0, 3.0))
make_bar_chart(ax, ['Clear glass','Frosted plastic','Cardboard'], [92,48,2],
               title='', ylabel='Light transmitted (%)', xlabel='Material')
save_graph(fig, 'u6_material_transmission.png')
plt.close(fig)

# 3. Light distance versus time in nanoseconds: d = 0.30 t.
fig, ax = plt.subplots(figsize=(3.5, 3.0))
light_distance = [{
    'expr': lambda t: 0.30*t,
    'deriv': lambda t: 0*t + 0.30,
    'color': 'steelblue',
    'label': None,
}]
make_context_graph(ax, light_distance, 0, 20, 0, 6,
                   xlabel='Time (ns)', ylabel='Distance (m)', title='')
save_graph(fig, 'u6_light_distance.png')
plt.close(fig)

# 4. Law of reflection relationship: reflected angle = incident angle.
fig, ax = plt.subplots(figsize=(3.5, 3.0))
reflection = [{
    'expr': lambda x: x,
...
make_context_graph(ax, reflection, 0, 80, 0, 80,
                   xlabel='Incident angle (deg)', ylabel='Reflected angle (deg)', title='')
save_graph(fig, 'u6_reflection_relation.png')
plt.close(fig)

# 5. Refractive index versus light speed, with speed measured in 10^8 m/s.
fig, ax = plt.subplots(figsize=(3.5, 3.0))
index_speed = [{
    'expr': lambda x: 3.0/x,
    'deriv': lambda x: -3.0/(x**2),
    'color': 'steelblue',
    'label': None,
}]
make_context_graph(ax, index_speed, 1.5, 3.0, 1.0, 2.1,
                   xlabel='Light speed (10^8 m/s)', ylabel='Index of refraction', title='')
save_graph(fig, 'u6_index_speed.png')
plt.close(fig)

# 6. Critical angle into air versus higher refractive index.
fig, ax = plt.subplots(figsize=(3.5, 3.0))
critical = [{
    'expr': lambda n: np.degrees(np.arcsin(1.0/n)),
    'deriv': lambda n: (-1.0/(n**2*np.sqrt(1-1/(n**2))))*(180/np.pi),
    'color': 'steelblue',
    'label': None,
}]
make_context_graph(ax, critical, 1.1, 2.5, 20, 70,
                   xlabel='Higher refractive index', ylabel='Critical angle (deg)', title='')
save_graph(fig, 'u6_critical_angle.png')
plt.close(fig)

# 7. Image height versus object height for a lens setup with magnification 2.
fig, ax = plt.subplots(figsize=(3.5, 3.0))
magnification = [{
    'expr': lambda h: 2*h,
    'deriv': lambda h: 0*h + 2,
    'color': 'steelblue',
    'label': None,
}]
make_context_graph(ax, magnification, 0, 5, 0, 10,
                   xlabel='Object height (cm)', ylabel='Image height (cm)', title='')
save_graph(fig, 'u6_lens_magnification.png')
plt.close(fig)
