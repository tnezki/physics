...
# =============================================================================
# UNIT 7 WARMUP FIGURES — APPENDED GENERATION BLOCKS ONLY
# =============================================================================

# 1. Unit 6 retrieval: wavelength-frequency relationship for visible light.
fig, ax = plt.subplots(figsize=(3.5, 3.0))
light_relation = [{
    'expr': lambda f: 3.0/f,
    'deriv': lambda f: -3.0/(f**2),
    'color': 'steelblue',
    'label': None,
}]
make_context_graph(ax, light_relation, 3, 7, 0.3, 1.1,
                   xlabel='Frequency (10^14 Hz)', ylabel='Wavelength (micrometers)', title='')
save_graph(fig, 'u7_u6_light_retrieval.png')
plt.close(fig)

# 2. Coulomb inverse-square relationship, normalized to F=1 at distance factor 1.
fig, ax = plt.subplots(figsize=(3.5, 3.0))
coulomb_distance = [{
    'expr': lambda r: 1.0/(r**2),
    'deriv': lambda r: -2.0/(r**3),
    'color': 'steelblue',
    'label': None,
}]
make_context_graph(ax, coulomb_distance, 1, 3, 0, 1.1,
                   xlabel='Distance factor', ylabel='Relative electric force', title='')
save_graph(fig, 'u7_coulomb_distance.png')
plt.close(fig)

# 3. Coulomb force versus one charge amount when distance and the other charge stay fixed.
fig, ax = plt.subplots(figsize=(3.5, 3.0))
coulomb_charge = [{
    'expr': lambda q: q,
    'deriv': lambda q: 0*q + 1,
    'color': 'steelblue',
    'label': None,
}]
make_context_graph(ax, coulomb_charge, 0, 3, 0, 3,
                   xlabel='Charge factor', ylabel='Relative electric force', title='')
save_graph(fig, 'u7_coulomb_charge.png')
plt.close(fig)

# 4. Charge passing a point versus time. Slope is current = 3 A.
fig, ax = plt.subplots(figsize=(3.5, 3.0))
charge_time = [{
    'expr': lambda t: 3*t,
    'deriv': lambda t: 0*t + 3,
    'color': 'steelblue',
    'label': None,
}]
make_context_graph(ax, charge_time, 0, 10, 0, 30,
                   xlabel='Time (s)', ylabel='Charge passed (C)', title='')
save_graph(fig, 'u7_charge_time.png')
plt.close(fig)

# 5. Ohmic resistor: V = 4 I, so slope is 4 ohms.
fig, ax = plt.subplots(figsize=(3.5, 3.0))
ohms = [{
    'expr': lambda i: 4*i,
    'deriv': lambda i: 0*i + 4,
    'color': 'steelblue',
    'label': None,
}]
make_context_graph(ax, ohms, 0, 6, 0, 24,
                   xlabel='Current (A)', ylabel='Voltage (V)', title='')
save_graph(fig, 'u7_ohms_graph.png')
plt.close(fig)

# 6. Resistance versus wire length for the same material and thickness.
fig, ax = plt.subplots(figsize=(3.5, 3.0))
res_length = [{
    'expr': lambda L: 2*L,
    'deriv': lambda L: 0*L + 2,
    'color': 'steelblue',
    'label': None,
}]
make_context_graph(ax, res_length, 0, 5, 0, 10,
                   xlabel='Wire length (m)', ylabel='Resistance (ohms)', title='')
save_graph(fig, 'u7_resistance_length.png')
plt.close(fig)

# 7. Device power ratings for simple retrieval/comparison.
fig, ax = plt.subplots(figsize=(4.0, 3.0))
make_bar_chart(ax, ['Tablet','Lamp','Heater'], [20,60,120],
               title='', ylabel='Power (W)', xlabel='Device')
save_graph(fig, 'u7_power_devices.png')
plt.close(fig)

# 8. Energy transferred versus time for two power levels.
fig, ax = plt.subplots(figsize=(3.5, 3.0))
power_time = [
    {
        'expr': lambda t: 40*t,
        'deriv': lambda t: 0*t + 40,
        'color': 'steelblue',
        'label': '40 W device',
    },
    {
        'expr': lambda t: 80*t,
        'deriv': lambda t: 0*t + 80,
        'color': 'firebrick',
        'label': '80 W device',
    },
]
make_context_graph(ax, power_time, 0, 5, 0, 400,
                   xlabel='Time (s)', ylabel='Energy transferred (J)', title='')
save_graph(fig, 'u7_power_time.png')
plt.close(fig)

# 9. Two device schedules with equal electrical energy use.
fig, ax = plt.subplots(figsize=(4.0, 3.0))
make_bar_chart(ax, ['100 W for 3 h','60 W for 5 h'], [300,300],
               title='', ylabel='Energy use (Wh)', xlabel='Operating schedule')
save_graph(fig, 'u7_energy_compare.png')
plt.close(fig)
